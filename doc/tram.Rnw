...
\Abstract{
The \pkg{tram} package allows a range of stratified linear
transformation models to be fitted using standard formula-based \proglang{R}
interfaces. Functions for the estimation of potentially stratified Cox models, several survival
regression models including log-normal or log-logistic models, 
models for ordered categorial responses (proportional odds, proportional
hazards, probit) are available. Likelihood-based inference, also in the presense of random
left-, right-, and interval-censoring and arbitrary forms of truncation, is
performed using infrastructure from package \pkg{mlt}. More complex models, allowing non-linear and interaction
functions of covariates, can be estimated using corresponding transformation
trees and forests in package \pkg{trtf}.}
...
models provided by package \pkg{mlt} \citep{pkg:mlt}.  The underlying
theory is presented by \cite{Hothorn_Moest_Buehlmann_2017} and an
introduction to the \pkg{mlt} package is given in the package vignette
of package \pkg{mlt.docreg} \cite{pkg:mlt.docreg}. An interface to
...
choices of the `link' function $\pZ$ and the monotone increasing (in it's $\ry$ argument) `baseline transformation' $\h_\rY$
...
(restricting the outself to very smooth regression coefficient functions
...
in essense, fit two unconditional models (for each treatment arm) at once
...
\end{eqnarray*}
so that we get $\log(\rY) \sim \ND((-\eparm_1, \shiftparm) / \eparm_2, 1 /
\eparm^2)$. 
...
in \cmd{clm} and \cmd{Polr} (we use treatment contrasts for $\tilde{s}$
here)
...
censoring (or truncation) might very well lead to substantically different
models.
...
coef(Coxph_GBSG2_1)
@
This model estimates a global treatment effect, \ie, a treatement effect
which applies uniformly to all patients. We now try to find out if this
model can be improved by a transformation tree
<<GBSG2-Cox-3>>=
...
In our last example we restrict out attention to an age-varying log-hazard ratio
...
<<funs, echo = FALSE, results='hide', purl = FALSE, cache = FALSE>>=
...
pkgs <- c("mlt", "variables", "basefun", "survival", "eha", "prodlim", "truncreg", "MASS", "nnet", "HSAUR3", "sandwich", 
  ...
  "ordinal", "VGAM", "BB", "alabama", "trtf", "mlt.docreg")
out <- sapply(pkgs, pkg)
