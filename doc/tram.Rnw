...
sapply(c("tram", "survival", "MASS", "lattice", "mlbench", 
         "multcomp", "ordinal", "colorspace", "quantreg"), library, char = TRUE)
...
BoxCox(y | 0 + chas + crim + zn + indus + nox + 
       ...
       data = BostonHousing2)
@
put postpone this exercise until Section~\ref{sec:colr}.
...
\begin{eqnarray*}
\Prob(\rY > \ry \mid \rX = \rx) = S(\ry \mid \rX = \rx) = \exp(-\exp(\eparm_1 + \eparm_2 \log(\ry) - \tilde{\rx}^\top \shiftparm))
\end{eqnarray*}
and because of $S(\ry) = \exp(-\Lambda(\ry))$ we get the conditional cumulative
hazard function
\begin{eqnarray*}
\Lambda(\ry \mid \rX = \rx) = \exp(\eparm_1 + \eparm_2 \log(\ry) - \tilde{\rx}^\top \shiftparm)
= \exp(\eparm_1 + \eparm_2 \log(\ry)) \exp(-\tilde{\rx}^\top \shiftparm)
\end{eqnarray*}
with baseline cumulative hazard function $\Lambda_\rY(\ry) = \exp(\eparm_1 + \eparm_2
\log(\ry))$ and hazard ratio $\exp(-\tilde{\rx}^\top \shiftparm)$, \ie the
relative change of the conditional cumulative hazard function compared to an
individual with $\tilde{\rx}^\top \shiftparm = 0$. The baseline hazard function
is then $\lambda_\rY(\ry) = \Lambda^\prime(\ry) = \exp(\eparm_1 +
\eparm_2 \log(\ry)) / (\eparm_2 \ry)$.

With $\eparm_2 = 1$, the distribution function simplifies to an exponential distribution
\begin{eqnarray*}
...
\cmd{survreg} uses a different parameterisation with scale parameter
$\sigma$:
\begin{eqnarray*}
...
2) * 100}\%$ the hazard of an untreated patient.

\begin{figure}
<<GBSG2-Weibull-1-plot>>=
nd <- data.frame(horTh = factor(c("no", "yes")))
plot(Survreg_GBSG2_1, newdata = nd, which = "distribution", type = "survivor", 
     confidence = "interval", fill = fill, col = col)
legend("bottomleft", lty = 1, title = "Hormonal Therapy", legend = levels(nd$horTh), bty = "n", col = col)
@
\end{figure}
...
\exp(\h_\rY(\ry))$. The term $\exp(- \tilde{\rx}^\top \shiftparm)$ is the
odds ratio. Typically (for example in \cmd{coxph}), the model is
parameterised as
\begin{eqnarray*}
\Prob(\rY \le \ry \mid \rX = \rx) = 1 - \exp(-\exp(\h_\rY(\ry) + \tilde{\rx}^\top \shiftparm))
\end{eqnarray*}
and thus the hazard ratio is $\exp(\tilde{\rx}^\top \shiftparm)$.
...
\begin{eqnarray*}
\Prob(\rY \le \ry \mid \rX = \rx) = \frac{\exp(\h_\rY(\ry) + \tilde{\rx}^\top
...
\end{eqnarray*}
where $\exp(\tilde{\rx}^\top \shiftparm)$ is the cutpoint $\ry$
independent odds ratio (compared to a subject with $\tilde{\rx}^\top
\shiftparm = 0$).

\begin{eqnarray*}
\Prob(\rY \le \ry \mid \rX = \rx) = \Phi(\h_\rY(\ry) - \tilde{\rx}^\top \shiftparm)
\end{eqnarray*}
is Box-Cox regression with $\Ex(\h_\rY(\rY) \mid \rX = \rx) = \tilde{\rx}^\top
\shiftparm$.
...
plot(contourplot(d ~ lp + q | chas, data = nd2, panel = pfun, xlab = "Linear predictor", 
     ylab = "Observed", main = "Continuous Outcome Logistic Regression"))
@
\end{figure}

\section{Connection to Quantile Regression}

Linear quantile regression for the $p$th conditional
quantile $Q_\rY(p \mid \rX = \rx)$ of $\rY$:
\begin{eqnarray*}
Q_\rY(p \mid \rX = \rx) = \tilde{\alpha}(p) + \tilde{\rx}^\top
\tilde{\shiftparm}(p)
\end{eqnarray*}
Quantiles are invariate wrt.~monotone transformations $\h_\rY$ and we have
\begin{eqnarray*}
\h_\rY(Q_\rY(p \mid \rX = \rx)) = Q_{\h_\rY(\rY)}(p \mid \rX = \rx)
\end{eqnarray*}

With $\ry = Q_\rY(p \mid \rX = \rx)$ we have
\begin{eqnarray*}
& & p = \Prob(\rY \le \ry \mid \rX = \rx) = \pZ(\h_Y(\ry) - \tilde{\rx}^\top\shiftparm) \\
& \iff & \h_\rY(Q_\rY(p \mid \rX = \rx)) = \pZ^{-1}(p) + \tilde{\rx}^\top\shiftparm
\end{eqnarray*}

When we allow for response varying effects 
\begin{eqnarray*}
& & p = \Prob(\rY \le \ry \mid \rX = \rx) = \pZ(\h_Y(\ry) - \tilde{\rx}^\top\shiftparm(\ry)) \\
& \iff & \h_\rY(Q_\rY(p \mid \rX = \rx)) = \pZ^{-1}(p) +
\tilde{\rx}^\top\tilde{\shiftparm}(p)
\end{eqnarray*}
with $\tilde{\shiftparm}(p) = \shiftparm(Q_\rY(p \mid \rX = \rx))$.

Fit linear quantile regression on a grid along with continuous outcome
logistic regression with response varying effects (``distribution
regression'').
...
Colr_BH_2 <- Colr(cmedv | crim + zn + indus + chas + nox + rm + age + dis + 
                  rad + tax + ptratio + b + lstat ~ 0, data = BostonHousing2, order = 2)
@
...
\section{Transformation Trees}

\section{Transformation Forests}
