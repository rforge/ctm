...
%\VignetteDepends{tram, survival, MASS, mlbench, multcomp, ordinal, colorspace, quantreg}
...
to computational infrastructure for conditional transformation models
provided by package \pkg{mlt}. \pkg{tram} restricts its attention to 
likelihood inference for stratified linear transformation models. An
interface to package \pkg{trtf} allows more complex models to be fitted
by recursive partitioning and random forest technology.

In a nutshell, the model class covered by the package consists of transformation 
models of the form
...
$\rx$, respectively. We use the term `stratum' is a little more general
...
\basisy(\ry)^\top\parm_2
\end{eqnarray*}
The coefficient function $\alpha_2(\ry)$ of $s$ is called `time-varying' in
...
linear regression models (\cmd{Polr}), and survival analysis (\cmd{Survreg},
...
as a nuisance parameter. This, the variance needs to be estimated simultaneously with the
...
standard deviations are parameters in our model, we can construct a test for
...
numeric covariates leads to the simplest form of `distribition regression',
...
@
\caption{\label{BostonHousing-BC-1-plot}}
\end{figure}
...
(indicating that there are hardly any cheap houses to find in this area).
...
@
\caption{\label{BostonHousing-BC-2-plot}}
\end{figure}

The heterogeneous variances in our model \code{Lm_BH_3} where caused by the
same effect and Figure~\ref{BostonHousing-Lm-3-plot} shows that us allowing
...
legend("bottomright", lty = 1, col = col, 
       title = "Near Charles River", legend = c("no", "yes"), bty = "n")
@
\caption{\label{BostonHousing-Lm-3-plot}}
\end{figure}
...
\shiftparm = 0$) is simply $\exp(\tilde{\rx}^\top \shiftparm)$. The
...
logLik(Colr_BH_1)
@
In comparion to the BoxCox-type model \code{BC_BH_2} with the exact same
...
pfun <- function(x, y, z, subscripts, at, ...) {
    ...
    panel.xyplot(x = -lp[nd$chas == ch], y = subset(BostonHousing2, chas == ch)$cmedv, pch = 20, 
                 col = rgb(.1, .1, .1, .1), ...)   
}
plot(contourplot(d ~ lp + q | chas, data = nd2, panel = pfun, xlab = "Linear predictor", 
     ylab = "Median Value"))#, main = "Continuous Outcome Logistic Regression"))
@
\caption{Continuous Outcome Logistic Regression \label{BostonHousing-Colr-1-plot}}
\end{figure}
...
transformation models. In essense, quantile regression (as the name
...
fitted for varying probabilities $p \in (0, 1)$, potentiall causing a
...
that a transformation model with response-varying effects (aka `distribtion
...
We first fit a numer of linear quantile regressiosn on a grid and, second, 
estimate a continuous outcome logistic regression with response-varying effects
(restricting the outself to very smooth regression coefficient functions by
...
Colr_BH_2 <- Colr(cmedv | crim + zn + indus + chas + nox + rm + age + dis + 
                  rad + tax + ptratio + b + lstat ~ 0, data = BostonHousing2, 
                  order = 2)
@
...
\begin{figure}
<<BostonHousing-rq-1-plot, echo = FALSE, fig.width = 10, fig.height = 10>>=
...
for (i in idx) {
  ...
  plot(Colr_BH_2, newdata = nd, which = "distribution", type = "distribution", 
       q = 5:36, main = paste("Obs.", i))
  points(qrq, tau, type = "b")
  arrows(BostonHousing2[i, "cmedv"], 0, BostonHousing2[i, "cmedv"], .2)
#  abline(v = BostonHousing2[i, "cmedv"])
}
@
\caption{Arrow indicates observed value. \label{BostonHousing-rq-1-plot}}
\end{figure}


\section{Survival Analysis}

Yet another choice of $\pZ$ is motivated by the desire of simple model
...
\cmd{survreg} uses a different parameterisation of the same model with scale parameter
of the form
$\sigma$:
\begin{eqnarray*}
...
Consider time-to-death data from the German Breast Cancer Study Group 2
...
plot(Survreg_GBSG2_1, newdata = nd, which = "distribution", type = "survivor", 
     confidence = "interval", fill = fill, col = col, ylab = "Probability")
legend("bottomleft", lty = 1, title = "Hormonal Therapy", legend = levels(nd$horTh), 
       bty = "n", col = col)
@
\caption{\label{GBSG2-Weibull-1-plot}}
\end{figure}

The question is simple to answer when we use the treatment as a stratum and,
in essense, fit two models at once
<<GBSG2-Weibull-2, cache = TRUE>>=
Survreg_GBSG2_2 <- Survreg(Surv(time, cens) | 0 + horTh ~ 1, data = GBSG2)
logLik(Survreg_GBSG2_2)
survreg_GBSG2_2 <- survreg(Surv(time, cens) ~ strata(horTh) + horTh - 1, data = GBSG2)
logLik(survreg_GBSG2_2)
...
odds ratio. Typically (for example in \cmd{coxph} and our \cmd{Coxph}), the model is
parameterised as
\begin{eqnarray*}
\Prob(\rY \le \ry \mid \rX = \rx) = 1 - \exp(-\exp(\h_\rY(\ry) + \tilde{\rx}^\top \shiftparm))
\end{eqnarray*}
and thus the hazard ratio is $\exp(\tilde{\rx}^\top \shiftparm)$. The
...
plot(survfit(Surv(time, cens) ~ horTh, data = GBSG2), col = col, 
     ylab = "Probability")
plot(Coxph_GBSG2_1, newdata = nd, which = "distribution", type = "survivor", 
     col = col, add = TRUE, lty = 1)
plot(Coxph_GBSG2_2, newdata = nd, which = "distribution", type = "survivor", 
     col = col, add = TRUE, lty = 2)
legend("bottomleft", lty = 1, title = "Hormonal Therapy", legend = levels(nd$horTh), 
       bty = "n", col = col)
@
\caption{\label{GBSG2-Cox-1-plot}}
\end{figure}
...
odds ratio $\exp(- \tilde{\rx}^\top \shiftparm)$.
...
\end{eqnarray*}
we have $\log(\rY) \sim \ND((-\eparm_1, \shiftparm) / \eparm_2, 1 / \eparm^2)$

Two ways to change the baseline transformation $\eparm_1 + \eparm_2
\log(\ry)$. Simplify to linear function $\eparm_1 + \eparm_2 \ry$ (and thus
to logistic or normal distributions of the outcome) or generalising to
$\h_\rY(\ry)$. This is what the Cox model does.
...
Polr_wine_2 <- Polr(rating | 0 + contact ~ temp, data = wine)
logLik(Polr_wine_2)
@

<<wine-clm-3, cache = TRUE>>=
clm_wine_3 <- clm(rating ~ temp, nominal = ~ contact, data = wine, link = "probit")
logLik(clm_wine_3)
@

<<wine-Polr-3, cache = TRUE>>=
Polr_wine_3 <- Polr(rating | 0 + contact ~ temp, data = wine, method = "probit")
logLik(Polr_wine_3)
@

<<wine-censored, cache = TRUE>>=
erating <- wine$rating
lrating <- erating
rrating <- erating
...
Polr_wine_4 <- Polr(R(erating, cleft = lrating, cright = rrating) | 0 + contact ~ temp, data = wine, method = "probit")
logLik(Polr_wine_4)
Polr_wine_4 <- Polr(crating | 0 + contact ~ temp, data = wine, method = "probit")
logLik(Polr_wine_4)
# summary(Polr_wine_4)
@
...
\section{Transformation Boosting}
...
\end{eqnarray*}
wher the parameters of the baseline transformation 
...
\begin{eqnarray*}
\Prob(\rY \le \ry \mid \rX = \rx, \text{horTh}) = \pZ(\basisy(\ry)^\top \parm(\rx) +
I(\text{horTh}) \beta(\rx))
\end{eqnarray*}
Here we are interested in an $\rx$-varying log-hazard ratio
$\beta(\rx)$. We first fit the model using a transformation tree
<<GBSG2-Cox-3>>=
Coxph_GBSG2_3 <- trafotree(Coxph_GBSG2_1, formula = Surv(time, cens) ~ horTh | age + menostat + tsize + 
    tgrade + pnodes + progrec + estrec, data = GBSG2)
logLik(Coxph_GBSG2_1)
logLik(Coxph_GBSG2_3)
coef(Coxph_GBSG2_3)[, "horThyes"]
@
Figure~\ref{GBSG2-Cox-3-plot} shows that the global treatment effect
vanishes in patients with many positive lymph nodes and a larger tumor
grading.
\begin{figure}
<<GBSG2-Cox-3-plot>>=
...
@
\caption{\label{GBSG2-Cox-3-plot}}
\end{figure}

Now we restrict out attention to an age-varying log-hazard ratio
$\beta(\text{age})$ and fit a more flexible model using a transformation
forest
<<GBSG2-Cox-4, cache = TRUE>>=
ctrl <- ctree_control(minsplit = 30, minbucket = 15, mincriterion = 0)
Coxph_GBSG2_4 <- traforest(Coxph_GBSG2_1, formula = Surv(time, cens) ~ horTh | age, control = ctrl, 
                           ntree = 100, mtry = 1, data = GBSG2)
@
The effect is plotted in \ref{GBSG2-Cox-4-plot}. Very young women seem to
profit most from the treatment, whereas women in the mid-forties seem not to
profit or even have an increased risk under hormonal therapy.
...
@
\caption{\label{GBSG2-Cox-4-plot}}
\end{figure}
