...
p <- predict(dvc_mlt, newdata = data.frame(1), q = q,
             type = "distribution")
plot(q, p, pch = 17, cex = .5, xlab = "Number of Roe Deer-Vehicle Collisions", 
     ylab = "Distribution", col = "grey")
points(q, ppois(q, lambda = mean(dvc)), pch = 18, cex = .5,
       col = "grey")
legend("bottomright", pch = c(17, 18), 
       legend = c("Transformation Model", "Poisson"), bty = "n",
       cex = .5, col = "grey")
@
...
b_hours <- as.basis(~ hours, data = PSID1976, ui = matrix(c(0, 1), nr  = 1), ci = 0)
ctm_PSID1976 <- ctm(b_hours, shift = fm_PSID1976[-2L], data = PSID1976_0, todistr = "Normal") 
mlt_PSID1976 <- mlt(ctm_PSID1976, data = PSID1976_0, scale = TRUE, check = FALSE)

logLik(tr_PSID1976)
logLik(mlt_PSID1976)

(cf2 <- coef(tr_PSID1976))
cf <- coef(mlt_PSID1976)
c(-cf[-grep("hours", names(cf))] / cf["hours"], 1 / cf["hours"])

cf3 <- c(-cf2[-grep("sigma", names(cf2))] / cf2["sigma"], 1 / cf2["sigma"])
cf3 <- cf3[c(1, length(cf3), 2:(length(cf3) - 1))]

logLik(mlt_PSID1976, parm = cf3)

tmp1 <- tmp2 <- PSID1976_0
tmp1$hours <- tmp1$hours$exact
tmp2$hours <- 0

h1 <- model.matrix(ctm_PSID1976, data = tmp1) %*% cf
# h11 <- predict(mlt_PSID1976)
h2 <- model.matrix(ctm_PSID1976, data = tmp1, deriv = c(hours = 1)) %*% cf
h3 <- model.matrix(ctm_PSID1976, data = tmp2) %*% cf

sum(log((dnorm(h1) * h2) / (1 - pnorm(h3))))

sum(dnorm(h1, log = TRUE) + log(h2) - log(1 - pnorm(h3)))
logLik(mlt_PSID1976, parm = cf)
@
...
can be estimated using a multinomial regression.  
<<iris-1>>=
multinom_iris <- multinom(Species ~ ., data = iris, trace = FALSE)
iris$oSpecies <- ordered(iris$Species)
b_Species <- as.basis(iris$oSpecies)
ctm_iris <- ctm(b_Species, interacting = as.basis(~ Sepal.Length + Sepal.Width + 
                Petal.Length + Petal.Width, data = iris), todistr = "Logistic")
