...
a cascade of increasingly complex transformation models that can be
estimated, compared and analysed in the maximum likelihood framework. 
Models for the unconditional or conditional distribution function of any
univariate response variable can be set-up and estimated in the same
...
truncation. The relatively dense highlevel implementation in the \proglang{R}
...
representation of a vector of explanator variables $\rx$ (\eg contrasts
...
\tilde{\rx}^\top \shiftparm$ and one estimates the intercept $\alpha$ and
the regression parameters $\shiftparm$ by minimisation of the squared error
...
\textit{defined} by \cite{Fisher_1934} \citep[another example of the
approximation winning over the basic concept, see][]{Lindsey_1996, Lindsey_1999}
\begin{eqnarray*}
...
models comes from the ability to alter $\pZ$ and $\h_\rY$.
...
\citep[using the estimator developed by][]{Hothorn_Moest_Buehlmann_2016}
and analysed in \proglang{R} using the \pkg{mlt} add-on package \citep{pkg:mlt}.  Before we
start looking at details, we illustrate the workflow by means of an example
from unconditional density estimation.
...
which will be discussed in Section~\ref{sec:trafo} and is specified using the 
\cmd{ctm} function as
...
specified without reference to the actual observations, maybe with the
exception of the domain of the Bernstein polynomial, and second, although
the model is fully parametric, the resulting density resembles the
flexibility of a non-parametric approach (details in
...
thus standard likelihood asymptotics work for $\hat{\parm}$
...
The distribution function on an at least ordered response $\rY \in \samY$ is
...
@
A short-cut for ordered factors is
...
<<CHFLS-1-mlt>>=
mlt_CHFLS_1 <- mlt(ctm_CHFLS_1, data = CHFLS)
@
...
choice again being not important for larger $M$)
...
which is evaluated at the observed counts only.
...
in Bavaria, Germany, between 2002-01-01 and 2011-12-13. The daily counts range between
$\Sexpr{min(dvc)}$ and $\Sexpr{max(dvc)}$. A model for the daily number of roe deer-vehicle collision
using a Bernstein polynomial of order $6$ as basis function is fitted using
<<dvc>>=
...
\begin{eqnarray*}
\pZ(\h(\ry | \rx)) = \pZ(\basisyx(\ry, \rx)^\top \parm) = \pZ(\basisy(\ry)^\top \parm_1 - \tilde{\rx}^\top \shiftparm) \, \text{with} \, \basisyx = (\basisy^\top, -\tilde{\rx}^\top)^\top.
\end{eqnarray*}
...
@
In order to fit the model using the \pkg{mlt} package, we need to 
...
<<CHFLS-2-ctm>>=
ctm_CHFLS_2 <- ctm(response = b_happy, shifting = b_R, todistr = "Logistic")
...
interest, \ie a positive continuous variable
<<GBSG2-1, echo = TRUE>>=
...
\citep[the positive shift being in line with the implementation in \cmd{coxph} in package \pkg{survival}][]{Therneau_Grambsch_2000,pkg:survival}
<<GBSG2-1-Cox>>=
...
under ten linear constraints, here with at most $3000$ iterations (argument to \cmd{BB::spg})
<<GBSG2-1-mlt>>=
...
of the log cumulative hazard function are practically equivalent (see Figure~\ref{GBSG2-1-fss-plot} also)
...
$\h_\rY(\ry) = \log(\ry)$ and $\pZ = \pMEV$ the exponential AFT model arises
which can be specified as the Cox model above, only the Bernstein basis for time 
needs to be replaced with a log-transformation and the corresponding parameter is restricted
to one in the estimation
<<GBSG2-2>>=
ly <- log_basis(GBSG2y, ui = "increasing", remove_intercept = TRUE)
...
mlt_GBSG2_2 <- mlt(ctm_GBSG2_2, data = GBSG2, fixed = c("log(y)" = 1), 
                   scale = TRUE)
@
The \cmd{survreg} (package \pkg{survival}) and \cmd{phreg} (package \pkg{eha}) functions 
...
@
If we allow for a scaled log-transformation $\h_\rY(\ry) = \eparm_1 \log(\ry)$, the Weibull
...
\end{eqnarray*}
and estimate \emph{all} model parameters $(\parm_1, \beta)$ simulaneously, \ie both the
...
and use a Bernstein polynomial of order $6$. The conditional transformation
model is now given by this transformation of the response, a linear predictor
...
For the normal linear model, the fit seems appropriate for observations with linear predictor 
...
of hours of work conditional on participation in the labor
...
mlt_PSID1976_1 <- mlt(ctm_PSID1976_1, data = PSID1976_0, 
                      scale = TRUE)
@
...
explanatory variables into account, \ie by treating health as a stratum variable but
...
@
There is a clear tendency of people being happier with better health. We now `adjust'
for age and income
<<CHFLS-4, cache = TRUE>>=
...
not allowing interactions between health and the other two variables).

\subsubsection{Continuous Response}

\paragraph{Survival Analysis: GBSG-2 Trial (Cont'd)}

The Cox model presented in Subsection~\ref{subsec:ltm} features one baseline
...
\end{eqnarray*}
Here, the transformation function $\bern{10}(\ry)^\top \parm_1$ and 
$\bern{5}(\ry)^\top \parm_2$ correspond to the untreated and treated groups,
respectively:
<<GBSG2-4>>=
...
\end{eqnarray*}
The model may include for response-varying coefficients (as defined by the basis $\basisy$) corresponding
to the bases $(\basisx_1^\top,\dots, \basisx_J^\top)$ and constant shift effects. Such a model
is set-up using \cmd{ctm} with arguments \code{response} ($\basisy$), \code{interacting} ($(\basisx_1^\top,\dots, \basisx_J^\top)$)
and \code{shifting} ($-\basisx_\text{shift}^\top$). It would be conceptually possible
...
mlt_CHFLS_5 <- mlt(ctm_CHFLS_5, data = CHFLS, 
    scale = TRUE, maxit = 10000, gtol = 1e-3)
predict(mlt_CHFLS_5, newdata = mkgrid(mlt_CHFLS_5), 
        type = "distribution")
logLik(mlt_CHFLS_5)
@
The log-likelihood and the fitted distribution are, of course, equivalent but the parameters
allow for a direct interpretation of the effect of health relative to the baseline 
...
logLik(mlt_CHFLS_6)
@
and finally we include all three variables (health, age and income) allowing for
...
model allowing for response-specific regression coefficients, the ordering of the response
...
$(\pMEV, (\bern{5}^\top, \I(\text{hormonal therapy}))^\top, (\parm_1^\top, \beta)^\top)$,
implements the transformation function $\h(\ry | \text{treatment}) =
\bern{5}(\ry)^\top \parm_1 + \I(\text{hormonal therapy}) \beta$ where
$\bern{5}^\top \parm_1$ is the log-cumulative baseline hazard function
parameterised by a Bernstein polynomial and $\beta \in \RR$ is the
log-hazard ratio of hormonal therapy.  The choice $M = 5$ is used to exactly
reproduce the results presented in \cite{Hothorn_Moest_Buehlmann_2016}
<<GBSG2-6>>=
B_GBSG2y_5 <- Bernstein_basis(var = GBSG2y, order = 5, ui = "increasing")
ctm_GBSG2_6 <- ctm(B_GBSG2y_5, shifting = ~ horTh, data = GBSG2, 
                   todistr = "MinExtrVal")
...
log-likelihood under five linear constraints. The model assumes proportional hazards, 
an assumption whose
...
(see Section~\ref{sec:asympt}). The $95\%$
...
predictive properties of age.  As the surivor functions are clearly larger
...
B_h <- Bernstein_basis(order = 3, ui = "incre",
    var = numeric_var("head", support = quantile(db$head, c(.1, .9)), 
                      bounds = range(db$head)))
B_a <- Bernstein_basis(order = 3, ui = "none",
    var = numeric_var("lage", support = quantile(db$lage, c(.1, .9)), 
...
mlt_BHi <- mlt(ctm_BHi, data = BostonHousing2, scale = TRUE, maxit = 5000, gtol = 1e-3)
logLik(mlt_BHi)
AIC(mlt_BHi)
@
Figure~\ref{fig:Boston-Housing-dr-plot} compares the fitted densities for the linear transformation
model with constant regression coefficients and this distribution regression model. For some
observations, the variance of the conditional distribution funtions seems to be smaller in this
more complex model.
...
### we want
#tmp$counts <- R(tmp$counts)
#cv <- numeric_var("counts", support = 0:5)
B_cs <- Bernstein_basis(var = numeric_var("coverstorey", support = 1:110), 
                        order = 4)
B_c <- as.basis(tmp$counts)
ctm_treepipit <- ctm(B_c, interacting = B_cs)
mlt_treepipit <- mlt(ctm_treepipit, data = tmp, maxit = 10000, gtol = 1e-3)
@
The left
...
implementing the \emph{most likely transformation} estimator following
\cite{Hothorn_Moest_Buehlmann_2016} closely.
...
The \cmd{mlt} function uses convenience interface \cmd{BB} to the spectral
projected gradient method implemented in the \code{spg()} function of
package \pkg{BB} \citep{Varadhan_Gilbert_2009} for maximising this
...
empricial cumulative distribtion function, a Kaplan-Meier or Turnbull estimate) via a
(constrained) linear regression of $Z = \pZ^{-1}(\Prob(\rY \le \ry))$ on the
...
(arguments \code{q} for the vector of grid poins or \code{K} for the number
...
\end{eqnarray*}
is evaluated for potentially large numbers $K$ ad $N_\text{new}$. Because in the
...
are more or less identical as are the coefficient tables
<<GBSG2-1-coxph-cmpr>>=  
cf <- coef(coxph_GBSG2_1)
...
using \cmd{multcomp::cftest} are also rather similar
...
linear function $\mA \hat{\parm}_N$ as implemented in package \cmd{multcomp}. Here $\mA$ is
...
in a finer grid of \code{ncheat} values for plotting purposes
<<geyser-w-band>>=
head(cb_w <- confband(mlt_w, newdata = data.frame(1), K = 20, cheat = 100))
@
...
\code{nrow(newdata)} random sample from $\Prob_\rZ$ and returns 
the intervals $\ry_k < \rZ < \ry_{k + 1}$ (\code{interpolate = FALSE}) or
a linear interpolation thereof (\code{interpolate = TRUE}). 


\paragraph{Density Estimation: Geyser Data (Cont'd)}

The model-based bootstrap analogon of the confidence band for the
...
These functions corresponding to log-likelihood ratios less then the $95\%$
...
from the bootstrap sample but does not deteriorate for probabilities close
to zero and one.
...
The computation framework implemented in package \pkg{mlt} allows fitting of
a large class of transformation models.  Many established \proglang{R}
add-on packages implement special cases, mostly in the context of survival
times.  The most prominent is the \pkg{survival} package
...
in \cmd{survreg}.  Parametric proportional hazards (\cmd{eha}) and accelerated failure time
...
Alternative likelihood approaches to transformation models often parameterise
the hazard or log-hazard function by some spline, including the 
\proglang{R} add-on packages \pkg{polspline} \citep{pkg:polspline}, 
\pkg{logspline} \citep{pkg:logspline}, \pkg{bshazard} \citep{pkg:bshazard} and
\pkg{gss} \citep{Gu_2014,pkg:gss}. Packages \pkg{muhaz} \citep{pkg:muhaz} 
and \pkg{ICE} \citep{pkg:ICE} implement kernel smoothing for hazard function
estimation, the latter package allows for interval censored responses. The
penalised maximum likelihood estimation procedure for simultaneous estimation
of the baseline hazard function and the regression coefficients in a Cox model
is available from package \pkg{survivalMPL} \citep{pkg:survivalMPL}.
...
these packages can be re-implemented using the computational infrastructure
offered by package \pkg{mlt}.  The availability of (at least) two
independent implementations allows package developers to validate their
implementation.  In fact, some errors in earlier development versions of
\pkg{mlt} could be detected by comparing model outputs.  Because the
implementation of maximum likelihood estimation for conditional
transformation models presented in this paper relies on a rather dense code
base ($200$ lines of pure \proglang{R} code in \pkg{variables}, $860$ lines
in \pkg{basefun} and $1450$ lines in \pkg{mlt}, all convenience functions
and user interfaces included), the likelihood of implementation errors is
smaller compared the likelihood of errors in any of the plethora of
alternative implementations of special linear transformation models (but not
zero, of course).
...
computations on unfitted models.  In addition, model coefficients can be
altered by the user, for example for computing conditional distribution or
density functions or for the evaluation of the log-likelihood at arbitrary
values of the parameters.  The \cmd{simulate} method for \code{mlt} objects
can always be used to draw samples from fitted (or unfitted) transformation
models.  Thus, the implementation of parametric bootstrap procedures is a
straighforward exercise.

The very flexible and powerful user interface of \pkg{mlt} will,
however, be incomprehensible for most of its potential users.  Because
transformation models seldomly receive the attention they deserve in
statistics courses, the documentation of \pkg{mlt} will likely also confuse
many statisticians, let alone ``data scientists''.  Future efforts will
concentrate on the implementation of convenience interfaces, \ie higher
level user interfaces for special forms of transformation models, that
resemble the traditional notational and naming conventions the statistical
...
\section{The variables Package} \label{app:variables}
...
not be bounded ($\pm \infty$ is allowed).  For generating equally-spaced
...
\section{The basefun Package} \label{app:basefun}
...
are available.  Basis functions can be joined columnwise using \cmd{c} or
...
@
The model matrix contains a \code{constraint} attribute \citep[using package \pkg{Matrix}][]{pkg:Matrix}
<<basefun-log-constr>>=
attr(X, "constraint")
@
...
As an example, a monotone increasing Bernstein polynomial $\bern{3}$ can 
...
@
Other constraints include a decreasing function (\code{ui = "decreasing"}), 
...
The polynomial defined by the basis functions and parameters and the first
...
description is sufficent. If \code{data} is given, \code{scale = TRUE}
scales each columns of the model matrix to the unit interval. Here is
...
\section{The mlt Package} \label{app:mlt}

\subsection{Specifying Response Observations} \label{subapp:R}

The generic function \cmd{R} is an extention of \cmd{Surv} (package \pkg{survival}) to possibly truncated
or categorical response variables with methods for objects of class 
...
$\ubar{\ry}$ the level preceeding $\bar{\ry}$ (which is missing when the
...
xl <- xt - sample(c(Inf, (0:(length(xt) - 2)) / length(xt)), replace = FALSE)
xr <- xt + sample(c(Inf, (0:(length(xt) - 2)) / length(xt)), replace = FALSE)
...
the approximation of a $\chi^2_{20}$ distribution by an unconditonal
...
<<mlt-chi-B>>=
## support for distribition
yvar <- numeric_var("y", support = qY(c(.001, 1 - .001)), bounds = c(0, Inf))
...
and log-likelhood (evaluated for the estimated and
true coefficients)
<<mlt-chi-model>>=
coef(mod)
coef(fmod)
...
approximating true transformation model and the estimated transformation
model are plotted in Figure~\ref{fig:mlt-chi-plot}.
...
pkgs <- c("survival", "eha", "prodlim", "truncreg", "MASS", "nnet", "HSAUR3", "sandwich", 
