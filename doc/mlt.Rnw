...
b_Sat <- as.basis(~ Sat, data = housing, remove_intercept = TRUE,
              ...
              ui = diff(diag(2)), ci = 0)
ctm_housing_1 <- ctm(b_Sat, todist = "Logistic")
mlt_housing_1 <- mlt(ctm_housing_1, data = housing, weights = housing$Freq, check = FALSE)

model.matrix(ctm_housing_1, data = data.frame(Sat = unique(housing$Sat)))
...
B_w <- Bernstein_basis(order = 8, var = wvar, ui = "incre")
@
The (in our case unconditional) transformation model is now fully described by
the parameterisation $\h(\ry) = \basisy(\ry)^\top \parm$ and $\pZ = \Phi$
...
for $M = 8$ and $\Sexpr{round(AIC(mlt_w_40),1)}$ for $M = 40$ in favor of the smaller model.
...
Estimate a proportional odds model
...
b_Infl <- as.basis(~ Infl, data = housing, remove_intercept = TRUE, 
              negative = TRUE)
ctm_housing_2 <- ctm(b_Sat, shift = b_Infl, todist = "Logi")
mlt_housing_2 <- mlt(ctm_housing_2, data = housing, weights = housing$Freq, check = FALSE)

logLik(polr_housing_2)
logLik(mlt_housing_2)

cbind(polr = c(polr_housing_2$zeta, coef(polr_housing_2)), mlt = coef(mlt_housing_2))

cbind(polr = sqrt(diag(vcov(polr_housing_2)))[c(3, 4, 1, 2)],
      mlt = sqrt(diag(vcov(mlt_housing_2))))
@
...
We estimate the conditional distribution of housholder satisfaction
for each level of the perceived degree of influence householders have on the
management of the property. Non-proportional odds model.

<<housing-3>>=
b_Infl <- as.basis(~ Infl - 1, data = housing)
ctm_housing_3 <- ctm(b_Sat, interacting = b_Infl, todist = "Logistic")
mlt_housing_3 <- mlt(ctm_housing_3, data = housing, weights = housing$Freq, check = FALSE)
...
\caption{GBSG-2 Trial. Estimated survivor functions 
         ...
         treatment groups. \label{GBSG2-strata-plot}}
\end{center}
\end{figure}
...
Estimate the deviation of the conditional distribution of householder satisfaction
compared to the baseline distribution of householder statisfaction with \code{low}
influence on property management.

<<housing-4>>=
b_Infl <- as.basis(~ Infl, data = housing)
ctm_housing_4 <- ctm(b_Sat, interacting = b_Infl, todist = "Logistic")
mlt_housing_4 <- mlt(ctm_housing_4, data = housing, weights = housing$Freq, check = FALSE)
predict(mlt_housing_4, newdata = data.frame(Infl = unique(housing$Infl)), 
        q = unique(housing$Sat), type = "distribution")
...
\caption{Tree Pipit Counts. Observations (left panel, the size of the points is
         ...
         GAM, right panel). \label{fig:treepipit-plot}}
\end{center}
\end{figure}
