...
\begin{figure}
\begin{center}
<<dvc-plot, echo = FALSE, fig.width = 5>>=
q <- support(dvcvar)[[1]]
...
\begin{figure}
\begin{center}
<<GBSG2-1-fss-plot, fig.width = 5, fig.height = 4, echo = FALSE>>=
...
\begin{figure}
\begin{center}
<<Boston-Housing-dr-plot, echo = FALSE, fig.width = 6, fig.heigth = 4>>=
q <- mkgrid(mvar, 100)[[1]]
...
\begin{figure}
\begin{center}
<<geyser-w-cbplot, echo = FALSE, cache = TRUE, fig.height = 4, fig.width = 6, results = "hide">>=
layout(matrix(1:2, ncol = 2))
...
for (i in 1:length(new_w)) {
    ...
    tmpmlt <- mlt(ctm_w, data = ngeyser, check = FALSE, scale = TRUE, theta = coef(mlt_w))
    ll[[i]] <- logLik(tmpmlt) - logLik(tmpmlt, parm = coef(mlt_w))
    pdist[[i]] <- predict(tmpmlt, newdata = data.frame(1), type = "distribution", q = q)
    pdens[[i]] <- predict(tmpmlt, newdata = data.frame(1), type = "density", q = q)
}
@
...
i <- which(ll < quantile(ll, prob = .95))
tpdist <- pdist[i]
tpdens <- pdens[i]
layout(matrix(1:2, ncol = 2))
