...
function must be monotone increasing and thus it makes sense to restrict all functions to be monotone.
<<BostonHousing-dr, cache = TRUE>>=
b <- as.basis(fm_BH[-2L], data = BostonHousing2, scale = TRUE)
ctm_BHi <- ctm(B_m, interacting = b, sumconstr = FALSE)
