...
\NEW{The general workflow of working with the \pkg{mlt} package and the
...
``survival analysis'', or ``categorical data analysis'' in this documentation
of \pkg{mlt} will likely also confuse many statisticians, let alone data or
subject-matter scientists.  \NEW{Current efforts concentrate on the
...
data, normal linear and non-normal linear models are available in package
\pkg{tram} \citep{pkg:tram}.  The corresponding package vignette
demonstrates how some of the model outputs presented in this paper can be
