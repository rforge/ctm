...
In our linear model, the intercept $\alpha$ and
...
In essense, the package is built around two important functions.  \cmd{ctm}
...
empricial cumulative distribution function, a Kaplan-Meier or Turnbull estimate) via a
