...
mlt_iris <- mlt(ctm_iris, data = iris, scale = TRUE, trace = TRUE, maxit = 5000)
predict(mlt_iris, newdata = iris, q = sort(unique(iris$oSpecies)), type = "density")
predict(multinom_iris, newdata = iris, type = "prob")
@
