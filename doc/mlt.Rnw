...
\paragraph{Distribution Estimation: Deer-vehicle Collisions}
...
legend("bottomright", pch = c(17, 18), 
       ...
       cex = .5, col = "grey")
@
\caption{Deer-vehicle Collisions. \label{fig:dvc-plot}}
\end{center}
\end{figure}
...
\subsubsection{Categorical Data Analysis: Iris Data}

<<iris-1>>=
multinom_iris <- multinom(Species ~ ., data = iris)
iris$oSpecies <- ordered(iris$Species)
b_Species <- as.basis(~ oSpecies, data = iris, remove_intercept = TRUE,
              contrasts.arg = list(oSpecies = function(n)
