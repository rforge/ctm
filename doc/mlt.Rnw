...
dvar <- numeric_var("duration", support = c(1.0, 5.0), add = c(-1, 1), bound = c(0, Inf))
@
...
ctm_CHFLS_1 <- ctm(b_happy, todist = "Logistic")
mlt_CHFLS_1 <- mlt(ctm_CHFLS_1, data = CHFLS, check = FALSE)

model.matrix(ctm_CHFLS_1, data = data.frame(R_happy = unique(CHFLS$R_happy)))
...
be obtained from the \cmd{model.matrix} method, along with
...
is defined using the \cmd{ctm} function.
...
<<geyser-w>>=
wvar <- numeric_var("waiting", support = c(40.0, 100), add = c(-5, 15), bound = c(0, Inf))
...
b_R <- as.basis(~ R_age + R_income, data = CHFLS, remove_intercept = TRUE, 
                   negative = TRUE)
ctm_CHFLS_2 <- ctm(b_happy, shift = b_R, todist = "Logi")
mlt_CHFLS_2 <- mlt(ctm_CHFLS_2, data = CHFLS, check = FALSE, scale = TRUE)

logLik(polr_CHFLS_2)
logLik(mlt_CHFLS_2)

cbind(polr = c(polr_CHFLS_2$zeta, coef(polr_CHFLS_2)), mlt = coef(mlt_CHFLS_2))
...
one could use
<<variables-c>>=
v_temp <- numeric_var("ztemp", desc = "Zurich daytime temperature", unit = "Celsius", 
                      support = c(-10.0, 35.0), add = c(-5, 5), 
                      bound = c(-273.15, Inf))
@
...
The \pkg{basefun} package \citep{pkg:basefun} implements Bernstein, Legendre, log and 
polynomial basis functions. In addition, facilities for treating arbitrary model
matrices as basis function are available. Basis functions can be joined columnwise 
using \cmd{c} or the box product can be generated using \cmd{b}. The definition of
basis functions does not require any actual observations, only variable descriptions
(see Appendix~\ref{app:variables}) are necessary. Each basis offers
\cmd{model.matrix} and \cmd{predict} methods.
...
pb <- polynomial_basis(xvar, coef = c(TRUE, TRUE, FALSE, TRUE))
...
\subsection{Logarithmis Basis}

<<basefun-log>>=
### set-up log-basis with intercept for positive variable
lb <- log_basis(xvar, remove_intercept = FALSE, ui = "increasing")
...
<<basefun-Bernstein>>=
bb <- Bernstein_basis(xvar, order = 3, ui = "increasing")
cf <- c(1, 2, 2.5, 2.6)
head(X <- model.matrix(bb, data = x), 10)
### check constraints
...
head(X <- model.matrix(blb, data = x), 10)
attr(X, "constraint")
@

<<basefun-b>>=
fb <- as.basis(~ g, data = factor_var("g", levels = LETTERS[1:2]))
### join them: we get one intercept and one deviation _functions_
bfb <- b(bern = bb, f = fb)
nd <- expand.grid(mkgrid(bfb, n = 10))
### evaluate bases
head(X <- model.matrix(bfb, data = nd), 10)
attr(X, "constraint")
