...
library("truncreg")
(truncregtobin <- truncreg(durable ~ age + quant, data = ttobin))

ttobin$durable <- R(ttobin$durable, tleft = 0)
pd <- as.basis(~ durable, data = tobin, ui = matrix(c(0, 1), nr  = 1), ci = 0)
mtobin <- ctm(pd, shift = ~ age + quant, data = tobin, todistr = "Normal")
mlttobin <- mlt(mtobin, data = ttobin, scale = TRUE, check = FALSE)

logLik(truncregtobin)
logLik(mlttobin)	

-coef(mlttobin)[c("(Intercept)", "age", "quant")] / coef(mlttobin)["durable"]
@

lm; Boston Housing
<<BostonHousing>>=
data("BostonHousing", package = "mlbench")
lmBostonHousing <- lm(medv ~ ., data = BostonHousing)

coef(lmBostonHousing)
summary(lmBostonHousing)$sigma
logLik(lmBostonHousing)

BostonHousing$medvc <- with(BostonHousing, Surv(medv, medv < 50))
mvar <- numeric_var("medvc", support = c(10.0, 40.0), bounds = c(0, Inf))
Bm <- Bernstein_basis(mvar, order = 6, ui = "increasing")
fmBostonHousing <- medvc ~ crim + zn + indus + chas + nox + rm + age + 
    dis + rad + tax + ptratio + b + lstat
mBostonHousing <- ctm(Bm, shift = as.basis(fmBostonHousing[-2L], data = BostonHousing, 
                                           remove_intercept = TRUE, negative = TRUE),
                      todistr = "Normal")
mltBostonHousing <- mlt(mBostonHousing, data = BostonHousing, scale = TRUE, 
                        check = FALSE)
logLik(mltBostonHousing)

survregBostonHousing <- survreg(fmBostonHousing, data = BostonHousing, dist = "gaussian")
logLik(survregBostonHousing)
cf <- coef(survregBostonHousing)
xv <- names(cf)[-1]
cbind(lm = coef(lmBostonHousing)[xv] / summary(lmBostonHousing)$sigma,
      survreg = cf[xv] / survregBostonHousing$scale,
      mlt = coef(mltBostonHousing)[xv])

cbind(lm = sqrt(diag(vcov(lmBostonHousing))[xv]),
survreg = sqrt(diag(vcov(survregBostonHousing))[xv]),
mlt = sqrt(diag(vcov(mltBostonHousing))[xv]))

@

\begin{figure}
\begin{center}
<<BostonHousing-plot, echo = FALSE, fig.width = 6, fig.height = 4>>=
q <- 3:52
m <- predict(lmBostonHousing, data = BostonHousing)
s <- summary(lmBostonHousing)$sigma
...
d <- predict(mltBostonHousing, newdata = BostonHousing, q = q, type="distribution")
lp <- c(predict(mltBostonHousing, newdata = BostonHousing, q = 0, terms = "bshift"))
