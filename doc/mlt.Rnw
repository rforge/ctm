...
(the positive shift being in line with the implementation in \cmd{coxph} in package \pkg{survival})
<<GBSG2-1-Cox>>=
...
fss_GBSG2_1 <- flexsurvspline(fm_GBSG2, data = GBSG2, scale = "hazard", k = 9,
...
\cmd{multinom} from package \pkg{nnet} for the iris data shows
<<iris-1>>=
...
storey is given in the right panel  
<<treepipit-gam>>=
library("mgcv")
...
the matrix of Bernstein basis functions evaluated on a grid of \code{n} waiting times
...
<<geyser-w-band>>=
head(cb_w <- confband(mlt_w, newdata = data.frame(1), n = 20, ncheat = 100))
@
...
<<basefun-c>>=
class(blb <- c(bern = bb, 
               log = log_basis(xvar, ui = "increasing", remove_intercept = TRUE)))
