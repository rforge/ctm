...
<<ctm-uncon-disc-mlt>>=
mody <- mlt(my, data = as.data.frame(gy), dofit = FALSE)
coef(mody) <- qnorm(cumsum(dy[-5L]))
@
...
ry <- do.call("c", simulate(mody, nsim = 10000))
ry <- ordered(ry, levels = 1:nlevels(yvar), labels = levels(yvar))
nd <- data.frame(y = ry)
m <- mlt(my, data = nd, check = FALSE)
@
...
cumsum(dy)
c(pnorm(coef(m)), 1)
predict(m, q = gy, type = "distribution")
pnorm(confint(m))
cbind(dy, diff(c(0, c(pnorm(coef(m)), 1))))
@
...
modw <- mlt(mw, data = geyser, check = FALSE)
@
and we compare the estimated distribution function 
<<geyser-w-distribution, echo = TRUE>>=
ndw$d <- predict(modw, newdata = ndw, type = "distribution")
@
...
mw <- ctm(Bw, todistr = "Normal")
modw40 <- mlt(mw, data = geyser, check = FALSE)
ndw$d2 <- predict(modw40, q = ndw$waiting, type = "distribution")
lines(ndw$waiting, ndw$d2, lty = 2)
legend("topleft", lty = 1:2, legend = c("M = 8", "M = 40"), bty = "n")
plot(ndw$waiting, predict(modw, q = ndw$waiting, type = "density"), type = "l")
lines(ndw$waiting, predict(modw40, q = ndw$waiting, type = "density"), lty = 2)
@
...
fm <- "horTh + age + menostat + tsize + tgrade + pnodes + progrec + estrec"
m <- ctm(By, shift = as.formula(paste("~",fm)), todistr = dist, data = GBSG2)
coxmod <- mlt(m, data = GBSG2, maxit = 3000, scale = TRUE, check = FALSE)
coef(coxmod)
AIC(coxmod)

coxmod2 <- coxph(as.formula(paste("Surv(time, cens) ~", fm)), data = GBSG2)
AIC(coxmod2)
cf <- coef(coxmod2)
n <- length(coef(coxmod))
n <- (n - length(cf) + 1):n

cbind(coef(coxmod2), coef(coxmod)[n])
cbind(sqrt(diag(vcov(coxmod))[n]), sqrt(diag(vcov(coxmod2))))
...
\begin{figure}[t]
\begin{center}
<<fig_7_cox, fig.width=3.5, echo = FALSE>>=
lim <- range(cf1$test$tstat[n]) * c(1, 1.375)
plot(cf1$test$tstat[n], cf2$test$tstat, xlab = "z-Statistic MLT exact likelihood", 
     ylab = "z-Statistic Cox partial likelihood", xlim = lim, ylim = lim, las = 1)
abline(a = 0, b = 1, col = "grey")
points(cf1$test$tstat[n], cf2$test$tstat)
text(cf1$test$tstat[n], cf2$test$tstat, labels =  names(cf2$test$coefficients), pos = 4,
     cex = .7)
@
\caption{GBSG-2. Comparison of $z$-statistics for all variables obtained from the partial likelihood 
         (function \code{coxph()} in package \pkg{survival}) and 
         the exact likelihood with parameterisation of the log-cumulative baseline hazard function. 
         \label{fig_7_cox}}
\end{center}
\end{figure}
