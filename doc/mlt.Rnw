...
$\infty$. Using this parameterisation we obtain the maximum likelihood estimator
for $\parm$ as
\begin{eqnarray*}
\hat{\parm}_N := \argmax_{\parm \in \Theta} \sum_{i = 1}^N
\log(\lik(\basisy^\top \parm | \rY \in \esAY_i))
\end{eqnarray*}
and the most likely transformation is $\hat{\h}_N := \basisy^\top \hat{\parm}_N$.
...
\hat{\parm}_N \times {\basisy^\prime}^\top \hat{\parm}_N$, for a discrete model
