...
%\VignetteDepends{variables, basefun, mlt, survival, eha, prodlim, truncreg, lattice, gridExtra, MASS, nnet, HSAUR3, sandwich, flexsurv, grid, latticeExtra, colorspace, multcomp, mgcv, coxinterval, ICsurv, polspline, logspline, bshazard, gss, MIICD, TransModel, dynsurv, timereg, survivalMPL, np, gamlss, Matrix, muhaz, ICE, ordinal, VGAM, AER, coin, gamlss.data, mlbench, alabama}
...
analysis}.  We describe how conditional transformation models can be
specified, fitted by maximising the likelihood \citep[using the estimator
developed by][]{Hothorn_Moest_Buehlmann_2016} and analysed in \proglang{R}
...
\cmd{mlt}.  This function maximises the log-likelihood function using the
...
respectively. Before we start looking at details, we illustrate the workflow by means of
...
of estimating a discrete density whose maximum likelihood estimator
...
\begin{eqnarray*}
\text{medv} \mid \rX = \rx \sim \ND(\tilde{\rx}^\top \shiftparm, \sigma^2)
\end{eqnarray*}
...
notes that the log-cumulative hazard function $\bern{10}(\ry)^\top \parm_1$
...
$\parm_2$ are a Bernstein polynomial with coefficients $\parm_1 + \parm_2$.
...
distribution functions seems to be smaller in this more complex model. There
