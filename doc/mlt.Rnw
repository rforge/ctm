...
@
\caption{Geyser data: Estimated density for duration time obtained from
...
@
\caption{Geyser data: Estimated distribution (left, also featuring the empirical
...
<<housing>>=
library("MASS")
polr_housing <- polr(Sat ~ Infl, weights = Freq, data = housing)
...
cbind(polr = c(polr_housing$zeta, coef(polr_housing)), mlt = coef(mlt_housing_1))

cbind(polr = sqrt(diag(vcov(polr_housing)))[c(3, 4, 1, 2)],
...
@
\caption{BostonHousing data: Predicted vs.~observed for the normal linear model (left) and the linear
...
@
\caption{GBSG-2 data. Estimated survivor functions 
...
timevarying deviation transformation

<<GBSG2-deviation>>=
B_GBSG2y_5 <- Bernstein_basis(var = GBSG2y, order = 5, ui = "incre")
b_horTh <- as.basis(~ horTh, data = GBSG2)
mod_GBSG2_6 <- ctm(B_GBSG2y_5, interacting = b_horTh, todistr = "MinExtrVal")
...
s <- mkgrid(mlt_GBSG2_6, 15)
s$y <- s$y[s$y > 100 & s$y < 2400]
nd <- expand.grid(s)
K <- model.matrix(mod_GBSG2_6, data = nd)
Kyes <- K[nd$horTh == "yes",]
Kyes[,1:6] <- 0  
gh <- glht(parm(coef(mlt_GBSG2_6), vcov(mlt_GBSG2_6)), Kyes)
ci <- confint(gh)
coxy <- s$y

m <- ctm(B_GBSG2y, shifting = ~ horTh, todistr = "MinExtrVal", data = GBSG2)
...
@
\caption{GBSG-2 data. 
         Verification of proportional hazards: The log-hazard ratio $\hat{\beta}$
