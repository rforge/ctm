...
This exact likelihood is another prominent example of the approximation (via the
...
workflow sketched in this section. Technical issues about the three packages
\pkg{variables}, \pkg{basefun} and \pkg{mlt} involved are explained in
Appendices~\ref{app:variables}, \ref{app:basefun} and \ref{app:mlt}.
...
@
A monotone increasing Bernstein polynomial of order $8$ for \code{waiting} in the interval 
...
Therefore, there seems to be a happyness conflict between getting older \emph{and} richer.
...
\end{eqnarray*}
and estimate \emph{all} model parameters $(\parm_1, \shiftparm)$ simulaneously, \ie both the
...
and use a Bernstein polynomial of order $6$. In principle, this approach is equivalent
...
$\bern{5}^\top \hat{\parm}_{2, N}$, together with a $95\%$ confidence band
...
ctm_GBSG2_7 <- ctm(B_GBSG2y_5, interacting = b_horTh, 
                   todistr = "MinExtrVal")
...
\code{maxit}, are given to \cmd{spg}.
...
times.  The most prominent one is the \pkg{survival} package
...
\pkg{survivalMPL} \citep{pkg:survivalMPL}.  Estimation of the unconstrainted log-hazard 
function theoretically attractive but too erratic estimates have to be dealt with by
...
but one has to deal with a constraint problem. Package \pkg{mlt} follows the 
latter approach.
...
the existing packages mentioned in this paper can be re-implemented using
...
implementation of parametric bootstrap procedures is a straighforward
exercise.
...
resemble the traditional notational and naming conventions from the statistical
...
@
using the \code{Matrix} object \citep[from package \pkg{Matrix},][]{pkg:Matrix} representing
...
description is sufficent. If \code{data} is a data frame of observations,
