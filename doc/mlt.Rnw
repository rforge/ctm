...
\begin{figure}
\begin{center}
<<geyser-w-cbplot, echo = FALSE, cache = TRUE, fig.width = 6, fig.height = 4, results = "hide">>=
layout(matrix(1:2, ncol = 2))
...
\begin{figure}
\begin{center}
<<geyser-w-simulate-plot, fig.width = 6, fig.height = 4, echo = FALSE>>=
...
\pkg{intcox} \citep{pkg_intcox}, \pkg{coxinterval} \citep{pkg:coxinterval} \pkg{ICsurv} \citep{pkg:ICsurv}

\pkg{polspline} \citep{pkg:polspline}, \pkg{logspline} \citep{pkg:logspline}, \pkg{bshazard} \citep{pkg:bshazard}

\pkg{gss} \citep{Gu_2014,pkg:gss} \pkg{MIICD} \citep{pkg:MIICD}

\pkg{TransModel} \citep{pkg:TransModel}

\pkg{dynsurv}, \pkg{timereg} \citep{Scheike_Martinussen_2006,Scheike_Zhang_2011,pkg:timereg}

\pkg{survivalMPL} \citep{pkg:survivalMPL}
...
pkgs <- c("survival", "eha", "prodlim", "truncreg", "MASS", "nnet", "HSAUR3", "sandwich", 
  ...
  "dynsurv", "timereg", "survivalMPL", "np", "gamlss")
out <- sapply(pkgs, pkg)
