...
\end{eqnarray*}
where model interpretation relies on $\Ex(\rY | \rX = \rx) = \alpha + \tilde{\rx}^\top \shiftparm$ and
one estimates the intercept $\alpha$ and the 
regression parameters $\shiftparm$ by minimisation of the squared error 
$(\rY - \alpha - \tilde{\rx}^\top \shiftparm)^2$. With some touch-up in notation, the model 
can be equivalently written as a model for a conditional distribution
\begin{eqnarray*}
\Prob(\rY \le \ry | \rX = \rx) = \Phi\left(\frac{\ry - \alpha - \tilde{\rx}^\top \shiftparm}{\sigma}\right) \text{ or } (\rY | \rX = \rx) \sim \ND(\alpha + \tilde{\rx}^\top\shiftparm, \sigma^2).
\end{eqnarray*}
...
is approximated by the density of the normal distribution. Because we always observe
...
$\h_\rY$ is non-linear in the latter case and only the shift $\tilde{\rx}^\top
\tilde{\shiftparm}$ is linear in $\tilde{\rx}$).  It is now also obvious that the Cox
model has nothing to do with censoring, let alone survival times $\rY > 0$. 
It is a model for the conditional distribution of a continuous response
$\rY \in \RR$ when it is appropriate to assume that the conditional hazard
function is scaled by $\exp(\tilde{\rx}^\top \tilde{\shiftparm})$.  For both the linear
...
transformation models \citep{Hothorn_Kneib_Buehlmann_2014}. In this latter class, the conditional distribution function is 
\begin{eqnarray*}
...
and analysed in \proglang{R} using the \pkg{mlt} add-on package.  Before we
...
Section~\ref{sec:mlt}) to studies from different domains.  We start with the
...
invariant with respect to the ordering of the levels.
...
predict(mlt_CHFLS_1, newdata = data.frame(1), type = "density", q = mkgrid(b_happy)[[1]])
...
\begin{figure}
\begin{center}
<<geyser-w-plot, echo = FALSE, cache = TRUE, fig.width = 6, results = "hide">>=
...
<<dvc>>=
dvcvar <- numeric_var("dvc", support = min(dvc):max(dvc))
B_dvc <- Bernstein_basis(dvcvar, order = 6, ui = "increasing")
dvc_mlt <- mlt(ctm(B_dvc), data = data.frame(dvc = dvc), check = FALSE)
@
...
There is clear evidence for overdispersion as the variance of the Poisson
model is much smaller than the variance estimated by the transformation model.
...
between 2002 and 2011. \label{fig:dvc-plot}}
\end{center}
\end{figure}
...
but in probabilities $\pY(\ry_k)$ for some $\ry_1, \dots, \ry_K$ only. The discrete 
...
\begin{eqnarray*}
\pZ(\h(\ry) - \tilde{\rx}^\top \shiftparm) = \pZ(\basisy(\ry)^\top \parm_1 - \tilde{\rx}^\top \shiftparm)
\end{eqnarray*}
...
supply negative linear predictors as $\Ex(\h(\rY) | \rX = \rx) = \tilde{\rx}^\top \shiftparm$.
...
\tilde{\rx}^\top)^\top, \parm)$, where $\tilde{\rx}$ contains the treatment
...
(the positive shift being in line with the implementation in \cmd{coxph})
<<GBSG2-1-Cox>>=
...
ctm_GBSG2_1 <- ctm(B_GBSG2y, shift = fm_GBSG2[-2L], 
                   todistr = "MinExtrVal", data = GBSG2)
@
...
by the partial likelihood, we estimate all model parameters simultaneously
under ten linear constraints
<<GBSG2-1-mlt>>=
system.time(mlt_GBSG2_1 <- mlt(ctm_GBSG2_1, data = GBSG2, maxit = 3000, scale = TRUE, check = FALSE))
@
...
\pkg{flexsurv} package \cite{pkg:flexsurv} shows that the two spline parameterisations
of the log cumulative hazard function are practically equivalent (see Figure~\ref{GBSG2-1-fss-plot} also), although
\cmd{mlt} is ten times faster than \cmd{flexsurvspline} for this simple Cox model
<<GBSG2-1-fss, cache = TRUE>>=
system.time(fss_GBSG2_1 <- flexsurvspline(fm_GBSG2, data = GBSG2, scale = "hazard", k = 9,
                              bknots = log(support(GBSG2y)$y)))
logLik(fss_GBSG2_1)
logLik(mlt_GBSG2_1)
cbind(coxph = cf, mlt = coef(mlt_GBSG2_1)[names(cf)],
      fss = coef(fss_GBSG2_1)[names(cf)])
diag(vcov(mlt_GBSG2_1))[names(cf)]
diag(vcov(fss_GBSG2_1))[names(cf)]
@
...
baseline transformation $\h(ry)$ to a possibly scaled log-transformation. With
$\h(\ry) = \log(\ry)$ and $\pZ = \pMEV$ the exponential AFT model arises
which can be specified as the Cox model above, only the Bernstein basis for time 
needs to be replaced with a log-transformation
<<GBSG2-2>>=
...
@
The \cmd{survreg} and \cmd{phreg} functions fit the same model, the results
...
@
If we allow for a scaled log-transformation $\h(\ry) = \eparm_1 \log(\ry)$, the Weibull
...
\begin{eqnarray*}
\text{medv} | \rX = \rx \sim \ND(\tilde{x}^\top \shiftparm, \sigma^2)
\end{eqnarray*}
is rather restrictive. We relax the model formulation without sacrificing
the simplicity of a linear predictor of the explanatory variables
in the linear transformation model
\begin{eqnarray*}
\Prob(\text{medv} \le \ry | \rX = \rx) = \Phi(\h(\ry) - \tilde{\rx}^\top \shiftparm)
...
sraightforward to evaluate the conditional distribution function, the likelihood
...
of the explanatory variables the model and the normal probability function; the \cmd{mlt}
...
shows the fitted values, \ie the linear predictor for each observation, and the observed response
...
\begin{figure}
\begin{center}
<<BostonHousing-plot, echo = FALSE, results = "hide", fig.width = 6>>=
q <- 3:52
...
\cite{Mroz_1976} analysed the University of Michigan Panel Study of Income
...
function for the response (we want to stay within the normal family)
and a shift term
<<PSID1976-mlt>>=
...
mlt_PSID1976_1 <- mlt(ctm_PSID1976_1, data = PSID1976_0, 
    scale = TRUE, check = FALSE)
@
The \cmd{mlt} function does a slightly better job at maximising the likelihood which explains
...
cbind(truncreg = coef(tr_PSID1976),
      mlt = c(-cf[-grep("hours", names(cf))] / cf["hours"], 1 / cf["hours"]))
@
Of course, we might want to question the normal assumption by allowing a potentially
non-linear transformation function
<<PSID1976-mlt-ctm>>=
hvar <- numeric_var("hours", support = range(PSID1976_0$hours$exact),
                    bounds = c(0, Inf))
B_hours <- Bernstein_basis(hvar, order = 6, ui = "increasing")
...
people are happier for all health levels (this, of course, due to the restrictive model
not allowing for interactions between health and the other two variables).
...
\end{eqnarray*}
Here, the transformation function $\bern{5}(\ry)^\top \parm_1$ and 
...
are show in Figure~\ref{GBSG2-strata-plot}, the low-dimensional Bernstein polynomials
...
the transformation model agree on a positive age effect $\hat{\shiftparm}$, 
...
is defined by basis functions of the form
\begin{eqnarray*}
\basisyx = (\basisy^\top \otimes (\basisx_1^\top,\dots, \basisx_J^\top), -\basisx_\text{shift}^\top).
\end{eqnarray*}
The model allows for response-varying coefficients (as defined by the basis $\basisy$) corresponding
...
mlt_CHFLS_5 <- mlt(ctm_CHFLS_5, data = CHFLS, check = FALSE, 
...
\cmd{multinom} from package \pkg{nnet} shows
<<iris-1>>=
fm_iris <- Species ~ Sepal.Length + Sepal.Width + Petal.Length + Petal.Width
multinom_iris <- nnet::multinom(fm_iris, data = iris, trace = TRUE)
logLik(multinom_iris)
iris$oSpecies <- ordered(iris$Species)
b_Species <- as.basis(iris$oSpecies)
ctm_iris <- ctm(b_Species, interacting = as.basis(fm_iris[-2L], data = iris), 
    todistr = "Logistic")
mlt_iris <- mlt(ctm_iris, data = iris, scale = TRUE, check = FALSE)
logLik(mlt_iris)
...
The Cox model for the comparison of the survivor distribution between the untreated and treated
group assuming proportional hazards, \ie the model $(\pMEV, (\bern{5}^\top, \I(\text{hormonal
...
ctm_GBSG2_6 <- ctm(B_GBSG2y_5, shift = ~ horTh, data = GBSG2, 
    todistr = "MinExtrVal")
...
avar <- numeric_var("age", support = range(GBSG2$age))
B_age <- Bernstein_basis(avar, order = 3)
b_horTh <- as.basis(GBSG2$horTh)
ctm_GBSG2_8 <- ctm(B_GBSG2y_5, interacting = b(horTh = b_horTh, age = B_age), 
    todistr = "MinExtrVal")
...
mlt_head <- mlt(ctm_head, data = db, maxit = 5000, check = FALSE, 
                scale = TRUE)
@
...
Response-varying coefficient model, or distribution regression:
\begin{eqnarray*}
\Prob(\text{medv} \le \ry | \rX = \rx) & = &  
  \Phi\left(\h(\ry) - \sum_{j = 1}^J \shiftparm_j(\text{medv}) \tilde{\rx}_j\right) \\
& = & \Phi\left(\bern{6}(\ry)^\top \parm_1 - \sum_{j = 1}^J \bern{6}(\ry)^\top \parm_{j + 1} \tilde{\rx}_j\right)
\end{eqnarray*}
with, or other words, using the basis function 
...
model with constant regression coefficients and this distribution regression model.

\begin{figure}
\begin{center}
<<Boston-Housing-dr-plot, echo = FALSE, fig.width = 6>>=
q <- mkgrid(mvar, 100)[[1]]
...
B_cs <- Bernstein_basis(order = 4, 
    var = numeric_var("coverstorey", support = 1:110))
...
empricial cumulative distribtion function or a Turnbull estimator) via a
(constrained) linear regression of $Z = \pZ^{-1}(\Prob(\rY \le \ry))$ on the
design matrix of the transformation model.
...
likely transformation is $\hat{\h}_N$, plug-in estimators for the
distribution and cumulative hazard functions are given by $\hatpY = \pZ
...
(arguments \code{q} or \code{n}) for continuous 
...
using the \cmd{estfun} method from package \pkg{sandwich}) and the standard errors of the
regression coefficients. The positive effect of income is ``significant'' by standard measures
(the classical coefficient table was obtained using \cmd{cftest} from package \pkg{multcomp})
<<CHFLS-2-cmpr-2>>=
...
%cftest(survreg_GBSG2_2, parm = names(cf))
%# cftest(mlt_GBSG2_2, parm = names(cf))
%@
...
%cftest(coxph_GBSG2_5)
%cftest(mlt_GBSG2_5, parm = names(cf))
%@
...
the matrix of Bernstein basis functions evaluated on a grid of waiting times
$\ry_1, \dots, \ry_K$. 
<<geyser-w-band>>=
head(cb_w <- confband(mlt_w, newdata = data.frame(1), ncheat = 100))
@
...
\begin{figure}
\begin{center}
<<geyser-w-cbplot, echo = FALSE, cache = TRUE, fig.width = 6, results = "hide">>=
layout(matrix(1:2, ncol = 2))
...
distribution function over a grid of \code{n} response values, draws \code{nsim} times
...
for (i in 1:length(new_w)) {
    ...
    mlt_i <- mlt(ctm_w, data = ngeyser, check = FALSE, scale = TRUE, 
                 theta = coef(mlt_w))
...
quantile of the $100$ log-likelihood ratios, \ie after removal of extreme
...
\begin{figure}
\begin{center}
<<geyser-w-simulate-plot, fig.width = 6, echo = FALSE>>=
...
\caption{Old Faithful Geyser. Model-based bootstrap for distribution (left, the green area depicts the
         asymptotic confidence band from Figure~\ref{}) and density (right) function for 
...
$250$ (\pkg{variables}), $920$ (\pkg{basefun}), $1520$ (\pkg{mlt})
lines of \proglang{R} code
...
The generic \cmd{R} is an extention of \cmd{Surv} to possibly truncated
...
## Bernstein polynom order ord
ord <- 15
## support for distribition
yvar <- numeric_var("y", support = qY(c(.001, 1 - .001)), bounds = c(0, Inf))
By <- Bernstein_basis(yvar, order = ord, ui = "increasing")

## set-up model with Z ~ N(0, 1)
m <- ctm(By, todistr = "Normal")
d <- as.data.frame(mkgrid(yvar, n = 500))
mod <- mlt(m, data = d, dofit = FALSE)

## compute _true_ transformation function
h <- function(x) qnorm(pY(x))
x <- seq(from = support(yvar)[["y"]][1], to = support(yvar)[["y"]][2], length.out = ord + 1)
## coefs for Bernstein are f(v / ord) v = 0...ord
## coef(mod) defines a Bernstein polynomial
## approximating h
### new mod is a real model (although not data-based!)
mlt::coef(mod) <- h(x)

## compute true density
d$dtrue <- dY(d$y)

### simulate from _approximated_ distribution; so this is
### the new "true" one
d$grid <- d$y
d$y <- simulate(mod)
fmod <- mlt(m, data = d, check = FALSE, scale = TRUE)

print(fmod)
summary(fmod)

### compare; should be more or less the same
coef(mod)
coef(fmod)
...
\begin{center}
<<mlt-Ex-plot>>=
### plot "true" (red line) and "approximated" density (green)
plot(mod, newdata = d, type = "density", col = "black", xlab = "y", ylab = "Density")
lines(d$grid, d$dtrue, col = "red")
lines(sort(d$grid), predict(fmod, q = sort(d$grid), type = "density"), col = "blue")
@
\caption{}
\end{center}
\end{figure}
