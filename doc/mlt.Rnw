...
@
The result is depicted in Figure~\ref{fig:geyser-plot}. The plot shows the well-known bimodal
...
\caption{Old Faithful Geyser. Estimated density for duration time obtained from
         ...
         was parameterised by means of a Bernstein polynomial. \label{fig:geyser-plot}}
\end{center}
\end{figure}
...
In this section we introduce a cascade of increasingly complex transformation models
...
\paragraph{Categorical Data Analysis: Copenhagen Housing Conditions Survey}

We can specify the discrete basis function $\basisy$ as
<<housing-uncond>>=
library("MASS")
polr_housing_1 <- polr(Sat ~ 1, weights = Freq, data = housing)
b_Sat <- as.basis(~ Sat, data = housing, remove_intercept = TRUE,
              contrasts.arg = list(Sat = function(n) 
                  contr.treatment(n, base = 3)),
              ui = diff(diag(2)), ci = 0)
mod_housing_1 <- ctm(b_Sat, todist = "Logistic")
mlt_housing_1 <- mlt(mod_housing_1, data = housing, weights = housing$Freq, check = FALSE)

model.matrix(mod_housing_1, data = data.frame(Sat = unique(housing$Sat)))

logLik(polr_housing_1)
logLik(mlt_housing_1)

cbind(polr = polr_housing_1$zeta, mlt = coef(mlt_housing_1))

cbind(polr = sqrt(diag(vcov(polr_housing_1))),
      mlt = sqrt(diag(vcov(mlt_housing_1))))

predict(polr_housing_1, newdata = data.frame(1), type = "prob")
xtabs(Freq ~ Sat, data = housing) / sum(housing$Freq)
@
...
@
with the empirical cumulative distribution function in the left panel of Figure~\ref{fig:geyser-w-plot}.
...
\caption{Old Faithful Geyser. Estimated distribution (left, also featuring the empirical
         ...
         of order $8$ and $40$.  \label{fig:geyser-w-plot}}
\end{center}
\end{figure}
...
The corresponding AIC values are $\Sexpr{round(AIC(mltw),1)}$ 
for $M = 8$ and $\Sexpr{round(AIC(mltw40),1)}$ for $M = 40$ in favor of the smaller model.
...
<<housing>>=
library("MASS")
polr_housing_2 <- polr(Sat ~ Infl, weights = Freq, data = housing)
b_Infl <- as.basis(~ Infl, data = housing, remove_intercept = TRUE, 
              negative = TRUE)
b_Sat <- as.basis(~ Sat, data = housing, remove_intercept = TRUE,
              contrasts.arg = list(Sat = function(n) 
                  contr.treatment(n, base = 3)),
              ui = diff(diag(2)), ci = 0)
mod_housing_1 <- ctm(b_Sat, shift = b_Infl, todist = "Logi")
mlt_housing_1 <- mlt(mod_housing_1, data = housing, weights = housing$Freq, check = FALSE)

logLik(polr_housing_2)
logLik(mlt_housing_1)

cbind(polr = c(polr_housing_2$zeta, coef(polr_housing_2)), mlt = coef(mlt_housing_1))

cbind(polr = sqrt(diag(vcov(polr_housing_2)))[c(3, 4, 1, 2)],
      mlt = sqrt(diag(vcov(mlt_housing_1))))
@
...
mod_GBSG2_1 <- ctm(B_GBSG2y, shift = fm_GBSG2[-2L], todistr = "MinExtrVal", data = GBSG2)
mlt_GBSG2_1 <- mlt(mod_GBSG2_1, data = GBSG2, maxit = 3000, scale = TRUE, check = FALSE)
...
mod_GBSG2_2 <- ctm(ly, shift = as.basis(fm_GBSG2[-2L], data = GBSG2, negative = TRUE), todistr = "MinExtrVal")
mlt_GBSG2_2 <- mlt(mod_GBSG2_2, data = GBSG2, fixed = c("log(y)" = 1), scale = TRUE, check = FALSE)
...
<<GBSG2-weibull>>=
mlt_GBSG2_3 <- mlt(mod_GBSG2_2, data = GBSG2, scale = TRUE, check = FALSE)
...
mod_tobin <- ctm(b_d, shift = ~ age + quant, data = tobin, todistr = "Normal")
mlt_tobin <- mlt(mod_tobin, data = ttobin, scale = TRUE, check = FALSE)
...
mod_BH <- ctm(B_m, shift = as.basis(fm_BH[-2L], data = BostonHousing, 
                                           remove_intercept = TRUE, negative = TRUE),
                      todistr = "Normal")
mlt_BH <- mlt(mod_BH, data = BostonHousing, scale = TRUE, 
                        check = FALSE)
@
...
\paragraph{Categorical Data Analysis: Housing Survey}

<<housing-strat>>=
b_Infl <- as.basis(~ Infl - 1, data = housing)
b_Sat <- as.basis(~ Sat, data = housing, remove_intercept = TRUE,
              contrasts.arg = list(Sat = function(n) 
                  contr.treatment(n, base = 3)),
              ui = diff(diag(2)), ci = 0)
mod_housing_2 <- ctm(b_Sat, interacting = b_Infl, todist = "Logistic")
mlt_housing_2 <- mlt(mod_housing_2, data = housing, weights = housing$Freq, check = FALSE)
predict(mlt_housing_2, newdata = data.frame(Infl = unique(housing$Infl)), 
        q = unique(housing$Sat), type = "distribution")
@
...
mod_GBSG2_4 <- ctm(B_GBSG2y, interacting = b_horTh, todistr = "MinExtrVal")
mlt_GBSG2_4 <- mlt(mod_GBSG2_4, data = GBSG2, check = FALSE)
@

\begin{figure}
\begin{center}
<<GBSG2-strat-plot, echo = FALSE, results = "hide">>=
...
\caption{GBSG-2 Trial. Estimated survivor functions 
         ...
         treatment groups. \label{GBSG2-strat-plot}}
\end{center}
\end{figure}
...
<<GBSG2-strat-age>>=
mod_GBSG2_5 <- ctm(B_GBSG2y, interacting = b_horTh, shifting = ~ age, data = GBSG2, todistr = "MinExtrVal")
(mlt_GBSG2_5 <- mlt(mod_GBSG2_5, data = GBSG2, check = FALSE, scale = TRUE))
...
<<housing-deviation>>=
b_Infl <- as.basis(~ Infl, data = housing)
b_Sat <- as.basis(~ Sat, data = housing, remove_intercept = TRUE,
              contrasts.arg = list(Sat = function(n) 
                  contr.treatment(n, base = 3)),
              ui = diff(diag(2)), ci = 0)
mod_housing_3 <- ctm(b_Sat, interacting = b_Infl, todist = "Logistic")
mlt_housing_3 <- mlt(mod_housing_3, data = housing, weights = housing$Freq, check = FALSE)
predict(mlt_housing_3, newdata = data.frame(Infl = unique(housing$Infl)), 
        q = unique(housing$Sat), type = "distribution")
cftest(mlt_housing_3)
@

\paragraph{Continuous Response}
...
mod_GBSG2_6 <- ctm(B_GBSG2y_5, shift = ~ horTh, data = GBSG2, todistr = "MinExtrVal")
mlt_GBSG2_6 <- mlt(mod_GBSG2_6, data = GBSG2, check = FALSE)
@
...
mod_GBSG2_7 <- ctm(B_GBSG2y_5, interacting = b_horTh, todistr = "MinExtrVal")
mlt_GBSG2_7 <- mlt(mod_GBSG2_7, data = GBSG2, check = FALSE)
@
...
nd <- expand.grid(s)
K <- model.matrix(mod_GBSG2_7, data = nd)
Kyes <- K[nd$horTh == "yes",]
Kyes[,1:6] <- 0  
...
\caption{GBSG-2 Trial. Verification of proportional hazards: The log-hazard ratio $\hat{\beta}$
         ...
         computed from a non-proportional hazards model. \label{GBSG2-deviation-plot}}
\end{center}
\end{figure}   
...
mod_GBSG2_7 <- ctm(B_GBSG2y, interacting = b(horTh = b_horTh, age = B_age), todistr = "MinExtrVal")
mlt_GBSG2_7  <- mlt(mod_GBSG2_7, data = GBSG2, check = FALSE)
@
...
s <- mkgrid(mlt_GBSG2_7, 100)
nd <- expand.grid(s)
nd$s <- c(predict(mlt_GBSG2_7, newdata = s, type = "survivor"))
