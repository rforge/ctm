...
dvar <- numeric_var("cdur", support = range(tobin$durable[tobin$durable > 0]), bounds = c(0, Inf))

tobin$cdur <- with(tobin, Surv(durable, durable > 0, type = "left"))
ld <- log_basis(dvar, ui = "increasing")
mtobin <- ctm(ld, shift = as.basis(~ age + quant, data = tobin), todistr = "Normal")
mlttobin <- mlt(mtobin, data = tobin, scale = TRUE, check = FALSE)

survregtobin <- survreg(Surv(durable, durable > 0, type = "left") ~ age + quant,
