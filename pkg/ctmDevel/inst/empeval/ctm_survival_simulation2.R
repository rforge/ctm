...
##********************************************************************
## CTM for survival data: Simulation 2
##********************************************************************

## The R code given below enables the reader to reproduce the simulation results 
## of Simulation 2 given in Section 3 of the main paper.
...
## True hazard and survivor function of the Weibull distribution
h <- function(t, b, c){
     c / (b^c) * t^(c-1)
}
...
## Non-informative continuous covariate x
...
## b1, c1: scale and shape parameter of the Weibull distribution for G1.
## b2, c2: scale and shape parameter of the Weibull distribution for G2.
## x: grid of x-values.
generate_data <- function(seed, n, p, b1, c1, b2, c2, x){
     set.seed(seed)
...
x_var <- seq(from = 0, to = 1, length = 200)
x_ges <- expand.grid(x = x_var, G = factor(c("G1", "G2"), levels = c("G1", "G2")))

## seeds
set.seed(43090)
seeds <- round(runif(100) * 10000, digits = 0)

## Generate lists of data sets
## n = 200: 200 obersvations per treatment group.
## b1 = 1.5, c1 = 3: Weibull parameters for G1.
## b2 = 1, c2 = 1: Weibull parameters for G2.
list_df_5 <- list_df_10 <- list_df_25 <- list_df_50 <- vector(mode = "list", 
                                                              length = 100)
for(i in 1:100){
  list_df_5[[i]] <- generate_data(seeds[i], n = 200, p = 0.05, b1 = 1.5, c1 = 3, 
                                  b2 = 1, c2 = 1, x = x_ges$x)
  list_df_10[[i]] <- generate_data(seeds[i], n = 200, p = 0.1, b1 = 1.5, c1 = 3,
                                   b2 = 1, c2 = 1, x = x_ges$x)
  list_df_25[[i]] <- generate_data(seeds[i], n = 200, p = 0.25, b1 = 1.5, c1 = 3,
                                   b2 = 1, c2 = 1, x = x_ges$x)
  list_df_50[[i]] <- generate_data(seeds[i], n = 200, p = 0.5, b1 = 1.5, c1 = 3,
                                   b2 = 1, c2 = 1, x = x_ges$x)
}
...
set.seed(12345) 
sT_G1_test <- rweibull(1000, scale = 1.5, shape = 3)
sT_G2_test <- rweibull(1000, scale = 1, shape = 1)
...
list_km5 <- lapply(list_df_5, FUN = function(x){prodlim(Surv(sT, event) ~ G, 
                                                        data = x)})
list_km10 <- lapply(list_df_10, FUN = function(x){prodlim(Surv(sT, event) ~ G, 
                                                          data = x)})
list_km25 <- lapply(list_df_25, FUN = function(x){prodlim(Surv(sT, event) ~ G, 
                                                          data = x)})
list_km50 <- lapply(list_df_50, FUN = function(x){prodlim(Surv(sT, event) ~ G, 
                                                          data = x)})

## Predict survival probabilities for observations in df_eval over 
## ngrid_eval_G1/ngrid_eval_G2:
list_probs_km5_G1 <- lapply(list_km5, FUN = function(x){
                            matrix(unlist(predict(x, 
                                          newdata = df_eval[df_eval$G == "G1",], 
                                          times = ngrid_eval_G1)), 
                                   ncol = length(ngrid_eval_G1), 
                                   byrow = TRUE)})
list_probs_km5_G2 <- lapply(list_km5, FUN = function(x){
                            matrix(unlist(predict(x, 
                                          newdata = df_eval[df_eval$G == "G2",], 
                                          times = ngrid_eval_G2)), 
                                   ncol = length(ngrid_eval_G2), 
                                   byrow = TRUE)})
list_probs_km10_G1 <- lapply(list_km10, FUN = function(x){
                             matrix(unlist(predict(x, 
                                           newdata = df_eval[df_eval$G == "G1",], 
                                           times = ngrid_eval_G1)), 
                                    ncol = length(ngrid_eval_G1), 
                                    byrow = TRUE)})
list_probs_km10_G2 <- lapply(list_km10, FUN = function(x){
                             matrix(unlist(predict(x, 
                                           newdata = df_eval[df_eval$G == "G2",], 
                                           times = ngrid_eval_G2)), 
                                    ncol = length(ngrid_eval_G2), 
                                    byrow = TRUE)})
list_probs_km25_G1 <- lapply(list_km25, FUN = function(x){
                             matrix(unlist(predict(x, 
                                           newdata = df_eval[df_eval$G == "G1",], 
                                           times = ngrid_eval_G1)), 
                                    ncol = length(ngrid_eval_G1), 
                                    byrow = TRUE)})
list_probs_km25_G2 <- lapply(list_km25, FUN = function(x){
                             matrix(unlist(predict(x, 
                                           newdata = df_eval[df_eval$G == "G2",], 
                                           times = ngrid_eval_G2)), 
                                    ncol = length(ngrid_eval_G2), 
                                    byrow = TRUE)})
list_probs_km50_G1 <- lapply(list_km50, FUN = function(x){
                             matrix(unlist(predict(x, 
                                           newdata = df_eval[df_eval$G == "G1",], 
                                           times = ngrid_eval_G1)), 
                                    ncol = length(ngrid_eval_G1), 
                                    byrow = TRUE)})
list_probs_km50_G2 <- lapply(list_km50, FUN = function(x){
                             matrix(unlist(predict(x, 
                                           newdata = df_eval[df_eval$G == "G2",], 
                                           times = ngrid_eval_G2)), 
                                    ncol = length(ngrid_eval_G2), 
...
## Replace "NA" by "0":
for(i in 1:100){
  list_probs_km5_G1[[i]][is.na(list_probs_km5_G1[[i]])] <- 0
  list_probs_km10_G1[[i]][is.na(list_probs_km10_G1[[i]])] <- 0
  list_probs_km25_G1[[i]][is.na(list_probs_km25_G1[[i]])] <- 0
  list_probs_km50_G1[[i]][is.na(list_probs_km50_G1[[i]])] <- 0
  list_probs_km5_G2[[i]][is.na(list_probs_km5_G2[[i]])] <- 0
  list_probs_km10_G2[[i]][is.na(list_probs_km10_G2[[i]])] <- 0
  list_probs_km25_G2[[i]][is.na(list_probs_km25_G2[[i]])] <- 0
  list_probs_km50_G2[[i]][is.na(list_probs_km50_G2[[i]])] <- 0
}
...
for(i in 1:100){
  ...
  surv_probs5 <- matrix(surv5$surv, ncol = 400, byrow = TRUE)
  ...
  surv_probs10 <- matrix(surv10$surv, ncol = 400, byrow = TRUE)
  probs_mat_G1 <- matrix(0, nrow = 1000, ncol = 1000)
  probs_mat_G2 <- matrix(0, nrow = 1000, ncol = 1000)
  for(k in 1:1000){
  ...
  surv_probs25 <- matrix(surv25$surv, ncol = 400, byrow = TRUE)
  probs_mat_G1 <- matrix(0, nrow = 1000, ncol = 1000)
  probs_mat_G2 <- matrix(0, nrow = 1000, ncol = 1000)
  for(k in 1:1000){
  ...
  surv_probs50 <- matrix(surv50$surv, ncol = 400, byrow = TRUE)
  probs_mat_G1 <- matrix(0, nrow = 1000, ncol = 1000)
  probs_mat_G2 <- matrix(0, nrow = 1000, ncol = 1000)
  for(k in 1:1000){
...
for(i in 1:100){
  ...
  surv_mat <- matrix(surv_5.2$surv, ncol = 200, byrow = TRUE)
  times_mat <- matrix(surv_5.2$time, ncol = 200, byrow = TRUE)
  ...
  surv_mat <- matrix(surv_10.2$surv, ncol = 200, byrow = TRUE)
  times_mat <- matrix(surv_10.2$time, ncol = 200, byrow = TRUE)
  pred_mat_G1 <- matrix(0, nrow = 1000, ncol = 1000)
  pred_mat_G2 <- matrix(0, nrow = 1000, ncol = 1000)
  for(j in 1:1000){
    pred_mat_G1[j,] <- stepfun(times_mat[j,], c(1, surv_mat[j,]))(ngrid_eval_G1)
  ...
  surv_mat <- matrix(surv_25.2$surv, ncol = 200, byrow = TRUE)
  times_mat <- matrix(surv_25.2$time, ncol = 200, byrow = TRUE)
  pred_mat_G1 <- matrix(0, nrow = 1000, ncol = 1000)
  pred_mat_G2 <- matrix(0, nrow = 1000, ncol = 1000)
  for(j in 1:1000){
    pred_mat_G1[j,] <- stepfun(times_mat[j,], c(1, surv_mat[j,]))(ngrid_eval_G1)
  ...
  surv_mat <- matrix(surv_50.2$surv, ncol = 200, byrow = TRUE)
  times_mat <- matrix(surv_50.2$time, ncol = 200, byrow = TRUE)
  pred_mat_G1 <- matrix(0, nrow = 1000, ncol = 1000)
  pred_mat_G2 <- matrix(0, nrow = 1000, ncol = 1000)
  for(j in 1:1000){
    pred_mat_G1[j,] <- stepfun(times_mat[j,], c(1, surv_mat[j,]))(ngrid_eval_G1)
...
## ngrid for estimation consists of ALL event and censoring time points 
list_ngrid5 <- lapply(list_df_5, FUN = function(x){sort(unique(x$sT))})
list_ngrid10 <- lapply(list_df_10, FUN = function(x){sort(unique(x$sT))})
list_ngrid25 <- lapply(list_df_25, FUN = function(x){sort(unique(x$sT))})
list_ngrid50 <- lapply(list_df_50, FUN = function(x){sort(unique(x$sT))})
...
save(list_ipcw5, file = "list_ipcw5_sim2.Rda")
save(list_ipcw10, file = "list_ipcw10_sim2.Rda")
save(list_ipcw25, file = "list_ipcw25_sim2.Rda")
save(list_ipcw50, file = "list_ipcw50_sim2.Rda")

## set up model formula: 
## A separate smooth function over time is estimated for each treatment group;
## an interaction surface of the non-informative covariate x and time is estimated:
...
save(list_pred_G1_mat_5, file = "list_pred_G1_mat_5_sim2.Rda")
save(list_pred_G2_mat_5, file = "list_pred_G2_mat_5_sim2.Rda")
...
save(list_pred_G1_mat_10, file = "list_pred_G1_mat_10_sim2.Rda")
save(list_pred_G2_mat_10, file = "list_pred_G2_mat_10_sim2.Rda")
...
save(list_pred_G1_mat_25, file = "list_pred_G1_mat_25_sim2.Rda")
save(list_pred_G2_mat_25, file = "list_pred_G2_mat_25_sim2.Rda")
...
save(list_pred_G1_mat_50, file = "list_pred_G1_mat_50_sim2.Rda")
save(list_pred_G2_mat_50, file = "list_pred_G2_mat_50_sim2.Rda")


## **************************** CLTM ****************************************

## Fit conditionally linear transformation model with Cox model flexibility:
source("mctm_cltm.R")
...
save(list_pred_G1_CLTM_5, file = "list_pred_G1_CLTM_5_sim2.Rda")
save(list_pred_G2_CLTM_5, file = "list_pred_G2_CLTM_5_sim2.Rda")
...
save(list_pred_G1_CLTM_10, file = "list_pred_G1_CLTM_10_sim2.Rda")
save(list_pred_G2_CLTM_10, file = "list_pred_G2_CLTM_10_sim2.Rda")
...
save(list_pred_G1_CLTM_25, file = "list_pred_G1_CLTM_25_sim2.Rda")
save(list_pred_G2_CLTM_25, file = "list_pred_G2_CLTM_25_sim2.Rda")
...
save(list_pred_G1_CLTM_50, file = "list_pred_G1_CLTM_50_sim2.Rda")
save(list_pred_G2_CLTM_50, file = "list_pred_G2_CLTM_50_sim2.Rda")
...
MAD <- function(surv_probs_G1, surv_probs_G2, ngrid_G1, ngrid_G2, N, model_name){

  sum_G1 <- 0 
  for(l in 1:(N/2)){
    sum_G1 <- sum_G1 + sum(abs(surv_probs_G1[l,] - Strue(ngrid_G1, b = 1.5, c = 3)))/
                               length(ngrid_G1)
  }
  ...
  sum_G2 <- 0
  for(l in 1:(N/2)){
    sum_G2 <- sum_G2 + sum(abs(surv_probs_G2[l,] - Strue(ngrid_G2, b = 1, c = 1)))/
                               length(ngrid_G2)
  }
...
list_MADs_km5 <- list_MADs_km10 <- list_MADs_km25 <- list_MADs_km50 <- 
vector(mode = "list", length = 100)

for(i in 1:100){
list_MADs_km5[[i]] <- MAD(list_probs_km5_G1[[i]], 
                          list_probs_km5_G2[[i]], 
                          ngrid_G1 = ngrid_eval_G1, 
                          ngrid_G2 = ngrid_eval_G2, N = 2000,
                          model_name = "KM")
list_MADs_km10[[i]] <- MAD(list_probs_km10_G1[[i]], 
                           list_probs_km10_G2[[i]], 
                           ngrid_G1 = ngrid_eval_G1, 
                           ngrid_G2 = ngrid_eval_G2, N = 2000,
                           model_name = "KM")
list_MADs_km25[[i]] <- MAD(list_probs_km25_G1[[i]], 
                           list_probs_km25_G2[[i]], 
                           ngrid_G1 = ngrid_eval_G1, 
                           ngrid_G2 = ngrid_eval_G2, N = 2000,
                           model_name = "KM")
list_MADs_km50[[i]] <- MAD(list_probs_km50_G1[[i]], 
                           list_probs_km50_G2[[i]], 
                           ngrid_G1 = ngrid_eval_G1, 
                           ngrid_G2 = ngrid_eval_G2, N = 2000,
                           model_name = "KM")
}

MAD_km5 <- matrix(unlist(list_MADs_km5), nrow = 2)
rownames(MAD_km5) <- c("G1", "G2")
mean_MAD_km5 <- rowMeans(MAD_km5)
MAD_km10 <- matrix(unlist(list_MADs_km10), nrow = 2)
rownames(MAD_km10) <- c("G1", "G2")
mean_MAD_km10 <- rowMeans(MAD_km10)
MAD_km25 <- matrix(unlist(list_MADs_km25), nrow = 2)
rownames(MAD_km25) <- c("G1", "G2")
mean_MAD_km25 <- rowMeans(MAD_km25)
MAD_km50 <- matrix(unlist(list_MADs_km50), nrow = 2)
rownames(MAD_km50) <- c("G1", "G2")
mean_MAD_km50 <- rowMeans(MAD_km50)
## Save MADs for box plots:
save(MAD_km5, MAD_km10, MAD_km25, MAD_km50, 
     file = "MADs_km_sim2_boxplot.Rda")

mean_MAD_km <- matrix(0, nrow = 2, ncol = 4)
rownames(mean_MAD_km) <- c("G1", "G2")
colnames(mean_MAD_km) <- c("5% censoring", "10% censoring", "25% censoring", 
                           "50% censoring") 
mean_MAD_km[,1] <- mean_MAD_km5
mean_MAD_km[,2] <- mean_MAD_km10
mean_MAD_km[,3] <- mean_MAD_km25
mean_MAD_km[,4] <- mean_MAD_km50
save(mean_MAD_km, 
     file = "mean_MAD_km_sim2.Rda")
...
save(MAD_cox5, MAD_cox10, MAD_cox25, MAD_cox50, 
     file = "MADs_cox_sim2_boxplot.Rda")
...
save(MAD_stratcox5, MAD_stratcox10, MAD_stratcox25, MAD_stratcox50, 
     file = "MADs_stratcox_sim2_boxplot.Rda")
...
save(mean_MAD_stratcox, 
     file = "mean_MAD_stratcox_sim2.Rda")
...
save(MAD_CTM5, MAD_CTM10, MAD_CTM25, MAD_CTM50, 
     file = "MADs_CTM_sim2_boxplot.Rda")
...
save(MAD_CLTM5, MAD_CLTM10, MAD_CLTM25, MAD_CLTM50, 
     file = "MADs_CLTM_sim2_boxplot.Rda")
...
##************************** KM
list_ls_km5 <- list_ls_km10 <- list_ls_km25 <- list_ls_km50 <- 
vector(mode = "list", length = 100)

for(i in 1:100){

  list_ls_km5[[i]] <- log_score_eval(df_eval$sT[1:1000], 
                                     df_eval$sT[1001:2000],
                                     list_probs_km5_G1[[i]], 
                                     list_probs_km5_G2[[i]],
                                     ngrid_G1 = ngrid_eval_G1, 
                                     ngrid_G2 = ngrid_eval_G2, "KM")
  list_ls_km10[[i]] <- log_score_eval(df_eval$sT[1:1000], 
                                      df_eval$sT[1001:2000],
                                      list_probs_km10_G1[[i]], 
                                      list_probs_km10_G2[[i]],
                                      ngrid_G1 = ngrid_eval_G1, 
                                      ngrid_G2 = ngrid_eval_G2, "KM")
  list_ls_km25[[i]] <- log_score_eval(df_eval$sT[1:1000], 
                                      df_eval$sT[1001:2000],
                                      list_probs_km25_G1[[i]], 
                                      list_probs_km25_G2[[i]],
                                      ngrid_G1 = ngrid_eval_G1, 
                                      ngrid_G2 = ngrid_eval_G2, "KM")
  list_ls_km50[[i]] <- log_score_eval(df_eval$sT[1:1000], 
                                      df_eval$sT[1001:2000],
                                      list_probs_km50_G1[[i]], 
                                      list_probs_km50_G2[[i]],
                                      ngrid_G1 = ngrid_eval_G1, 
                                      ngrid_G2 = ngrid_eval_G2, "KM")
}
ls_km5 <- matrix(unlist(list_ls_km5), nrow = 2)
rownames(ls_km5) <- c("G1", "G2")
mean_ls_km5 <- rowMeans(ls_km5)
ls_km10 <- matrix(unlist(list_ls_km10), nrow = 2)
rownames(ls_km10) <- c("G1", "G2")
mean_ls_km10 <- rowMeans(ls_km10)
ls_km25 <- matrix(unlist(list_ls_km25), nrow = 2)
rownames(ls_km25) <- c("G1", "G2")
mean_ls_km25 <- rowMeans(ls_km25)
ls_km50 <- matrix(unlist(list_ls_km50), nrow = 2)
rownames(ls_km50) <- c("G1", "G2")
mean_ls_km50 <- rowMeans(ls_km50)
## save log scores for boxplots
save(ls_km5, ls_km10, ls_km25, ls_km50, 
     file = "ls_km_sim2_boxplot.Rda") 

mean_ls_km <- matrix(0, nrow = 2, ncol = 4)
rownames(mean_ls_km) <- c("G1", "G2")
colnames(mean_ls_km) <- c("5% censoring", "10% censoring", "25% censoring", 
                          "50% censoring") 
mean_ls_km[,1] <- mean_ls_km5
mean_ls_km[,2] <- mean_ls_km10
mean_ls_km[,3] <- mean_ls_km25
mean_ls_km[,4] <- mean_ls_km50

save(mean_ls_km, 
     file = "mean_ls_km_sim2.Rda")
...
save(ls_stratcox5, ls_stratcox10, ls_stratcox25, ls_stratcox50, 
     file = "ls_stratcox_sim2_boxplot.Rda") 
...
save(mean_ls_stratcox, 
     file = "mean_ls_stratcox_sim2.Rda")
...
Mean_MAD <- rbind(mean_MAD_CTM[1,], mean_MAD_CLTM[1,], mean_MAD_cox[1,], 
                  mean_MAD_stratcox[1,], mean_MAD_km[1,], mean_MAD_cf[1,],
                  mean_MAD_CTM[2,], mean_MAD_CLTM[2,], mean_MAD_cox[2,], 
                  mean_MAD_stratcox[2,], mean_MAD_km[2,], mean_MAD_cf[2,])
rownames(Mean_MAD) <- c("CTM G1", "CLTM G1", "Cox G1", "StratCox G1", 
                        "KM G1", "Cforest G1", "CTM G2", "CLTM G2", "Cox G2", 
                         "StratCox G2", "KM G2", "Cforest G2")
Mean_MAD <- Mean_MAD * 100
save(Mean_MAD, file = "Mean_MAD_sim2.Rda")

## Mean log score
mean_log_scores <- rbind(mean_ls_CTM[1,], mean_ls_CLTM[1,], mean_ls_cox[1,], 
                         mean_ls_stratcox[1,], mean_ls_km[1,], mean_ls_cf[1,],
                         mean_ls_CTM[2,], mean_ls_CLTM[2,], mean_ls_cox[2,], 
                         mean_ls_stratcox[2,], mean_ls_km[2,], mean_ls_cf[2,])
rownames(mean_log_scores) <- c("CTM G1", "CLTM G1", "Cox G1", "StratCox G1", 
                               "KM G1", "Cforest G1", "CTM G2", "CLTM G2", 
                               "Cox G2", "StratCox G2", "KM G2", "Cforest G2")
mean_log_scores <- mean_log_scores * 100
save(mean_log_scores, file = "mean_log_scores_sim2.Rda")
