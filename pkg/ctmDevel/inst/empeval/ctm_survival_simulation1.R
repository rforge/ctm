...
##********************************************************************
## CTM for survival data: Simulation 1
##********************************************************************

## The R code given below enables the reader to reproduce the simulation results 
## of Simulation 1 given in Section 3 of the main paper.
...
seeds[47] <- 486 ## change seed since previous seed leads to a problematic
                 ## data set.


## Generate lists of data sets
## n = 200: 200 obersvations per treatment group.
## b1 = 1, c1 = 3: Weibull parameters for G1.
## b2 = 1.5, c2 = 3: Weibull parameters for G2.
list_df_5 <- list_df_10 <- list_df_25 <- list_df_50 <- vector(mode = "list", 
                                                              length = 100)
for(i in 1:100){
  list_df_5[[i]] <- generate_data(seeds[i], n = 200, p = 0.05, b1 = 1, c1 = 3, 
                                  b2 = 1.5, c2 = 3, x = x_ges$x)
  list_df_10[[i]] <- generate_data(seeds[i], n = 200, p = 0.1, b1 = 1, c1 = 3,
                                   b2 = 1.5, c2 = 3, x = x_ges$x)
  list_df_25[[i]] <- generate_data(seeds[i], n = 200, p = 0.25, b1 = 1, c1 = 3,
                                   b2 = 1.5, c2 = 3, x = x_ges$x)
  list_df_50[[i]] <- generate_data(seeds[i], n = 200, p = 0.5, b1 = 1, c1 = 3,
                                   b2 = 1.5, c2 = 3, x = x_ges$x)
}
...
set.seed(12345) 
sT_G1_test <- rweibull(1000, scale = 1, shape = 3)
sT_G2_test <- rweibull(1000, scale = 1.5, shape = 3)
...
## Predict survival probabilites for observations in df_eval and over the
## grid of time points ngrid_eval_G1/ngrid_eval_G2:
...
## Estimate Cox models and predict survival probabilites for observations in df_eval
...
## Estimate conditional random forests and predict survival probabilites for
...
## Kaplan-Meier estimates of the censoring distribution:
...
list_ipcw5 <- list_ipcw10 <- list_ipcw25 <- list_ipcw50 <- vector(mode = "list", 
                                                                  length =100)
for(i in 1:100){
  list_ipcw5[[i]] <- inv_weights_graf(ngrid = list_ngrid5[[i]], sT = list_df_5[[i]]$sT,
                                      event = list_df_5[[i]]$event, G = list_G5[[i]])
  list_ipcw10[[i]] <- inv_weights_graf(ngrid = list_ngrid10[[i]], 
                                       sT = list_df_10[[i]]$sT, 
                                       event = list_df_10[[i]]$event, G = list_G10[[i]])
  list_ipcw25[[i]] <- inv_weights_graf(ngrid = list_ngrid25[[i]], 
                                       sT = list_df_25[[i]]$sT, 
                                       event = list_df_25[[i]]$event, G = list_G25[[i]])
  list_ipcw50[[i]] <- inv_weights_graf(ngrid = list_ngrid50[[i]], 
                                       sT = list_df_50[[i]]$sT, 
                                       event = list_df_50[[i]]$event, G = list_G50[[i]])
}
save(list_ipcw5, file = "list_ipcw5.Rda")
save(list_ipcw10, file = "list_ipcw10.Rda")
save(list_ipcw25, file = "list_ipcw25.Rda")
save(list_ipcw50, file = "list_ipcw50.Rda")
...
## Estimate models and predict survival probabilites for observations in df_eval
## over ngrid_eval_G1/ngrid_eval_G2:
...
save(list_pred_G1_mat_5, file = "list_pred_G1_mat_5.Rda")
save(list_pred_G2_mat_5, file = "list_pred_G2_mat_5.Rda")
...
save(list_pred_G1_mat_10, file = "list_pred_G1_mat_10.Rda")
save(list_pred_G2_mat_10, file = "list_pred_G2_mat_10.Rda")
...
save(list_pred_G1_mat_25, file = "list_pred_G1_mat_25.Rda")
save(list_pred_G2_mat_25, file = "list_pred_G2_mat_25.Rda")
...
save(list_pred_G1_mat_50, file = "list_pred_G1_mat_50.Rda")
save(list_pred_G2_mat_50, file = "list_pred_G2_mat_50.Rda")


## **************************** CLTM ****************************************

## Estimate conditionally linear transformation model with Cox model flexibility.
source("mctm_cltm.R")
fm2 <- "bbs(sT, df = 4, constraint = \"increasing\") ~ NULL"
fm2 <- as.formula(fm2)

## Estimate models and predict survial probabilites for observations in df_eval
## over ngrid_eval_G1/ngrid_eval_G2:
...
save(list_pred_G1_CLTM_5, file = "list_pred_G1_CLTM_5.Rda")
save(list_pred_G2_CLTM_5, file = "list_pred_G2_CLTM_5.Rda")
...
save(list_pred_G1_CLTM_10, file = "list_pred_G1_CLTM_10.Rda")
save(list_pred_G2_CLTM_10, file = "list_pred_G2_CLTM_10.Rda")
...
save(list_pred_G1_CLTM_25, file = "list_pred_G1_CLTM_25.Rda")
save(list_pred_G2_CLTM_25, file = "list_pred_G2_CLTM_25.Rda")
...
save(list_pred_G1_CLTM_50, file = "list_pred_G1_CLTM_50.Rda")
save(list_pred_G2_CLTM_50, file = "list_pred_G2_CLTM_50.Rda")
...
MAD <- function(surv_probs_G1, surv_probs_G2, ngrid_G1, ngrid_G2, N, model_name){
  ...
  sum_G1 <- 0 
  for(l in 1:(N/2)){
    sum_G1 <- sum_G1 + sum(abs(surv_probs_G1[l,] - Strue(ngrid_G1, b = 1, c = 3)))/
                               length(ngrid_G1)
  }
  ...
  sum_G2 <- 0
  for(l in 1:(N/2)){
    sum_G2 <- sum_G2 + sum(abs(surv_probs_G2[l,] - Strue(ngrid_G2, b = 1.5, c = 3)))/
                               length(ngrid_G2)
  }
...
save(MAD_cox5, MAD_cox10, MAD_cox25, MAD_cox50, 
     file = "MADs_cox_sim1_boxplot.Rda")
...
save(MAD_CTM5, MAD_CTM10, MAD_CTM25, MAD_CTM50, 
     file = "MADs_CTM_sim1_boxplot.Rda")
...
save(MAD_CLTM5, MAD_CLTM10, MAD_CLTM25, MAD_CLTM50, 
     file = "MADs_CLTM_sim1_boxplot.Rda")
...
save(mean_ls_CLTM, 
     file = "mean_ls_CLTM_sim1.Rda")



## ****************************** Tabulars for the paper:

## Mean MAD
Mean_MAD <- rbind(mean_MAD_CTM[1,], mean_MAD_CLTM[1,], mean_MAD_cox[1,], 
                  mean_MAD_km[1,], mean_MAD_cf[1,],
                  mean_MAD_CTM[2,], mean_MAD_CLTM[2,], mean_MAD_cox[2,], 
                  mean_MAD_km[2,], mean_MAD_cf[2,])
rownames(Mean_MAD) <- c("CTM G1", "CLTM G1", "Cox G1", "KM G1", "Cforest G1", 
                        "CTM G2", "CLTM G2", "Cox G2", "KM G2", "Cforest G2")
Mean_MAD <- Mean_MAD * 100
save(Mean_MAD, file = "Mean_MAD_sim1.Rda")

## Mean log score
mean_log_scores <- rbind(mean_ls_CTM[1,], mean_ls_CLTM[1,], mean_ls_cox[1,], 
                         mean_ls_km[1,], mean_ls_cf[1,],
                         mean_ls_CTM[2,], mean_ls_CLTM[2,], mean_ls_cox[2,], 
                         mean_ls_km[2,], mean_ls_cf[2,])
rownames(mean_log_scores) <- c("CTM G1", "CLTM G1", "Cox G1", "KM G1", "Cforest G1",
                               "CTM G2", "CLTM G2", "Cox G2", "KM G2", "Cforest G2")
mean_log_scores <- mean_log_scores * 100
save(mean_log_scores, 
     file = "mean_log_scores_sim1.Rda")
