...
& = & \Phi(\tilde{\alpha}_1 + \tilde{\alpha}_2 \ry - \tilde{\rx}^\top \tilde{\shiftparm})
\end{eqnarray}
...
such that $\Ex(\h_\rY(\rY) \mid \rX = \rx) = \tilde{\rx}^\top \shiftparm$.
...
are $\tilde{\shiftparm} = \shiftparm / \sigma$ and not $\shiftparm$
<<BostonHousing-coef>>=
coef(lm_BH)
cf <- coef(as.mlt(Lm_BH_1))
- cf[-2] / cf[2]
@
...
nd[, colnames(nd) != "chas"] <- 0
nd$rad <- nd$tax <- 0L
nd$chas <- factor(c("0", "1"))
col <- c("red", "blue")
plot(as.mlt(BC_BH_1), newdata = nd, q = 0:40, type = "trafo", lty = 1:2, col = col)
