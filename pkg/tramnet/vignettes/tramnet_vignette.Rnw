...
a non-parametric \cmd{independence\_test} from package \pkg{coin} can be used \citep{coinjss}.
First, the uncoditional transformation model \code{m0} is fit. Afterwards, the tramnet
...
residuals are independent from either lcp or age, we can conclude that it is
