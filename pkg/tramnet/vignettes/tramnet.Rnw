...
%\VignetteDepends{tramnet, lattice, colorspace, penalized, glmnet, mvtnorm, Matrix, kableExtra, coin, trtf, tbm, tram}
...
\abstract{
  The \pkg{tramnet} package implements regularized linear 
...
introduced in Section~\ref{subsec:tram}, for a more theoretical treatise we
refer to \citet{ctmpaper, mltpaper, mltjss}.  Convex optimization and domain
specific languages are briefly discussed in Section~\ref{subsec:CVXR},
followed by a treatment of model-based optimization for hyperparameter
tuning (\ref{subsec:mbo}).
...
$F\left(\h(\ry \given \rs, \rx)\right)$ has to be monotone increasing in $\ry$ and thus in
$\h$ for all possible strata $\rs$ and all possible configurations of the
covariates $\rx$. To ensure monotonicity, $\h$ is parametrized in terms of a basis
expansion using Bernstein polynomials as implemented in the \pkg{basefun}
...
censoring and truncation for continuous and discrete or ordered categorical
responses.
...
\begin{eqnarray*}
  \begin{aligned}
  	\ell_i(\parm,\shiftparm;\ry_i, \rs_i, \rx_i) &=
  	\left\{
  	\begin{array}{lll}
...
The likelihood $\sum_i \ell_i(\parm, \shiftparm; \ry_i, \rs_i, \rx_i)$ for
...
% \end{exmpl}
%-------------------------------------------------------------------------------
Because the penalty term $$\llpen$$ is convex in $\shiftparm$, it can
...
Thus, the optimisation routines implemented in package \pkg{CVXR} can be
...
the resulting optimization problem. In future versions \pkg{CVXR}, disciplined
...
log-transformation applied by \citet{zou2005regularization}. The linear models in \code{lpsa} and $\log($\code{psa}$)$
...
@
Because there is substantial evidence against independence of the models' residuals 
to either lcp or age, we can conclude that it is
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\bibliography{kook-hothorn,packages}
