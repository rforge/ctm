...
the zero $p$-vector for the left hand side, as done in the following example.
<<addconst>>=
m0 <- BoxCox(lpsa ~ 1, data = Prostate, extrapolate = TRUE)
mt <- tramnet(m0, x, alpha = 0, lambda = 0, constraints = list(diag(rep(1, 8)),
                                                               rep(0, 8)))
coef(mt)
@
...
for the two non-positive coefficient estimates, due to the convexity of the
underlying problem.
<<addconst_tram_cmp>>=
