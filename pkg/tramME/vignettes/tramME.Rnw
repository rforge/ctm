...
\abstract{
  ...
  accommodate correlated responses, the model is extended by incorporating
  mixed-effects. This article presents the R package \pkg{tramME}, which builds on
  existing implementations of transformation models (\pkg{mlt} and \pkg{tram}
...
\vspace{.5cm}
\noindent
{\color{red} A version of this article is submitted for publication to The R Journal.}
...
measurements all can lead to grouped data structures. The common property of
these data structures is that observations within groups defined by one or more
...
examples of the usage of mixed-effects models. Several R packages exist that
implement mixed-effects models for specific types of regression problems. The
two most notable examples are \CRANpkg{nlme} by \citet{pinheiro_nlme} and
...
approach. Most recently, \citet{tian_2020} reviewed the approach followed in
this study and compared to an alternative semiparametric formulation
using extensive simulations. With introducing random effects in the linear
transformation model framework, the model becomes applicable in a very diverse
set of regression problems, where the observations are correlated due to
repeated measurements or grouped designs.

\section{Mixed-effects transformation models}

The model class in the R package \pkg{tramME} is an extension of the
transformation model approach described by \citet{hothorn_most_2018} and implemented in
the R packages \CRANpkg{mlt} and \CRANpkg{tram} by \citet{hothorn_mlt}
and \citet{Hothorn2020_tram}, respectively.
...
\end{align}
%
where $F_{Z}$ denotes an pre-specified error distribution function (or
...
zero mean and the covariance matrix $\bSigma$, which is typically has a sparse
block structure.
...
models. In the R package \pkg{tramME}, seven main model types are
...
stratification, i.e defining separate transformation functions depending on the
value of a stratification variable, $s$. $h(y; \bvartheta)$ then becomes
$h(y\mid s;\bvartheta^{\prime})$. By stratifying to numeric variables, we are
allowing for response-varying effect sizes. Note that stratification leads to a
larger number of parameters in the baseline transformation function.
...
the outcome is that it is simple to introduce (independent) censoring and
truncation in the estimation process. The conditional likelihood contributions
...
methods. The add-on package \pkg{tramME} applies the Laplace approximation to
...
The maximum likelihood estimation in \pkg{tramME} is done using the \CRANpkg{TMB}
package by \citet{kristensen_tmb}. Template Model Builder (\pkg{TMB}) allows
the user to define and estimate general, non-linear mixed effects models. It was
built on well-tested and high-performance C++ libraries, which results in
a flexible yet efficient framework for estimating mixed models with possibly
complex random effects structures. See for example \citet{Brooks2017_glmmTMB}
for performance comparisons in the context of the package \CRANpkg{glmmTMB}. In
\pkg{tramME}, \pkg{TMB} is used to evaluate the integral in
Equation~\eqref{eq:lik}, using Laplace's method, and to calculate the
derivatives of the log-likelihood function using automatic (or algorithmic)
differentiation.
...
outcome variable, i.e. $h(y) = \vartheta_{1} + \vartheta_{2}y$, we arrive at a
re-parameterized version of the normal linear mixed effects model, and hence the
results from \code{tramME::LmME()} are directly comparable to estimates using
other mixed effects regression packages such as \pkg{lme4}. Estimating the
normal linear model with the \pkg{tramME}:
%
<<message=FALSE>>=
library("tramME")
...
supports the maximum likelihood estimation of the normal linear mixed model
specification.
%
<<message=FALSE>>=
library("lme4")
...
that it allows for incorporating independently censored outcomes. Let us assume
that the device used in the sleep deprivation study is only able to measure
reaction times larger than 200 ms and only in 50 ms step sizes. If the want to
take this reduced accuracy in measuring the reaction times into account, we have
to deal with interval-censored observations, as ignoring the censored nature of
the outcomes could lead to biased parameter estimates.
...
%
<<>>=
sleep_lm3 <- LmME(Reaction_ic ~ Days + (Days || Subject), data = sleepstudy)
logLik(sleep_lm3)
@
%
\noindent
Comparing the two models using a likelihood ratio test, we see no evidence
against the more parsimonious model (\code{sleep\_lm3}).
%
<<>>=
anova(sleep_lm2, sleep_lm3)
@
...
\citet{hothorn_most_2018} uses Bernstein bases to approximate the this general
increasing function in a fully parametric manner, i.e. $h(y) = \a_{\text{Bs},
K+1}(y)\T\bvartheta$. The resulting model can be regarded as a version of the
Box-Cox regression \citep{box_analysis_1964}, where the transformation of the
response is estimated simultaneously with the model parameters.
...
Plotting the baseline transformation , $h(y) = \a(y)\T\bvartheta$, in
Figure~\ref{fig:comp_lm_bc} reveals a slight departure from the conditional
normality of the outcome variable.

\begin{figure}[htbp]
  \begin{center}
<<sleepstudy-trafo, echo=FALSE,fig.width=4.5, fig.height=4, out.width="0.75\\linewidth">>=
...
plot(tr_bc, lty = 1, xlab = "Average reaction time (ms)", ylab = expression(h(y)),
     lwd = 2)
tr_lm <- trafo(sleep_lm, type = "trafo", confidence = "none", K = 100)
...
distribution of the outcomes, i.e. integrating out the random effects from the
conditional model~\eqref{eq:memodel}. In the general case, there is no
analyitical solution of the integral, but we can use numerical methods to
...
The increased flexibility of the Box-Cox-type model, i.e. using a general
baseline transformation function instead of a linear one, comes with the price
that the coefficient estimates will not be easily interpretable as expected
changes in the mean in the conditional model. Switching to the standard logistic
error distribution provides a solution to this problem, as the parameter estimates
in the resulting model can be interpreted as log-odds ratios. This
\emph{continuous outcome logistic regression} model was used by
...
placebo-contolled trial of low-level laser therapy for subjects with chronic
...
logodds of experiencing smaller pain levels as a linear function of fixed and
...
the model above reveals that this model is indeed a mixed-effects transformation
...
Figure~\ref{fig:colr_cmp} compares the results of the mixed-effects transformation model
approach to the estimates obtained using the \pkg{ordinalCont} package. Because
the two models are not exactly the same, we see some differences in the
parameter estimates as well as in the fitted baseline transformation function,
but the two model fits are reasonably close to each other.
...
\caption{Left: Baseline transformations in continuous outcome logistic
  ...
  and the areas indicate the 95\% point-wise confidence intervals. Right:
  Coefficient estimates from \pkg{tramME} and \pkg{ordinalCont} packages and
  their 95\% Wald confidence intervals.}
\label{fig:colr_cmp}
\end{figure}
...
%
\noindent
as well as the results from the \pkg{ordinalCont} package, suggest that there
is an imbalance in the sample at baseline, i.e. the odds of experiencing less
pain in the active treatment group is only about 10\% that of in the control
group for any pain levels. \todo{is this a correct interpretation?} Based on
the estimates, the treatment has strong significant effect, especially at the
seven-week follow-up, which seems to level off after 12 weeks.
...
## Saved for faster execution
mp_colr <-
structure(list(vas = c(0, 0.0101010101010101, 0.0202020202020202,
0.0303030303030303, 0.0404040404040404, 0.0505050505050505, 0.0606060606060606,
0.0707070707070707, 0.0808080808080808, 0.0909090909090909, 0.101010101010101,
"vas=0.89898990", "vas=0.90909091", "vas=0.91919192", "vas=0.92929293",
"vas=0.93939394", "vas=0.94949495", "vas=0.95959596", "vas=0.96969697",
"vas=0.97979798", "vas=0.98989899", "vas=1.00000000"), time = c("time=1",
...
To check visually the appropriateness of the proportional hazards assumption
between treatment and control groups, we re-estimate the model stratifying for
the treatment indicator, i.e. fitting baseline cumulative hazards for the
treatment and control groups separately, and check whether the two baseline
transformation functions, which are the log-cumulative baseline hazards when the
error distribution is the minimum extreme value distribution, are parallel.
%
...
that the treatment effect is constant over time, i.e. the proportionality
assumption is appropriate in the original model specification.
...
distributions are close to the Weibull distribution, i.e. we can substitute the
general baseline transformation function with $h(y) = \vartheta_1 +
\vartheta_2\log(y)$. Flipping the signs of the fixed and random effects terms of
\eqref{eq:coxph} and substituting the log-linear function to the baseline
...
The \code{SurvregME()} function of the \pkg{tramME} package implements a
variety of parametric mixed-effects models that represent specific choices of
the error distribution and the baseline transformation function in the general
formulation of Equation~\eqref{eq:memodel}.
...
those from the mixed effect Weibull model,
%
<<cache=TRUE>>=
## --- CoxphME
...
%
\noindent
confirms our suspicion that the dataset was simulated from a conditional Weibull model.

Finally, we might want to compare the results from \pkg{tramME} to parameters
estimated with the R package \pkg{coxme}.
%
...
\todo{Should I include another example (e.g. kidney data) to demonstrate
  identical results from parfm and frailtypack?}
...
\caption{Data from the soup tasting study reported by
  ...
  sure'') after tasting test and reference products. The datapoints are grouped
  by how often the respondent consume soup (more than once a week, one to four times
  a month, less than once a month).}
\label{fig:soup}
\end{figure}

Let us assume that we are interested in comparing the distribution of sureness
...
last one, $i=1\dots, 185$ indexing of the respondents, and $j=0,1$
...
%
\noindent
The R package \CRANpkg{ordinal} by \citet{christensen_ordinal} also
implements mixed effects regression models for ordered discrete outcomes. As a
cross-check can re-estimate the same model with the function \code{clmm()},
%
...
%
\noindent
The likelihood ratio test comparing the two specifications suggests that there
is some evidence that the extended, partially proportional model fits the data
better.
%
<<>>=
...
intensive steps of the maximum likelihood estimation. Howewer, a disadvantage of
this approach is that \pkg{TMB} uses Laplace approximation to integrate over
the vector of random effects. As several sources point out, Laplace's method can
lead to biased estimates in some distributional settings.
\citet{Pinheiro2006_AGQ} provides detailed numerical comparisons of the
Laplacian approximation to adaptive Gaussian quadrature algorithms in the
context of multilevel generalized models. Evaluating the method in the case of
discrete outcome mixed effects models, \citet{Joe2008} concludes that the
inaccuracy increases with the amount of discreteness of the response variable,
and decreases as the cluster sizes increase.

It is worth mentioning that the conditional approach of modeling the
distribution of the outcome, which is the basis of the transformation models
...
structure as nuisance parameters. Generalized estimating equations (GEE,
\citealp{hardin_generalized_2013}) models constitute prominent examples of such
an approach. Proponents of marginal models point out that the fixed effects
parameter estimates cannot be interpreted as population averages which is
usually of primary interest of a regression analysis.
...
distribution very often lead to complicated and implausible conditional
...
the case of marginal models. As we demonstarted in Sections~\ref{sec:boxcox} and
